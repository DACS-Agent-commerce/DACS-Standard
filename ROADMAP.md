...
| Item | Status | Notes |
|------|--------|-------|
| DACS client SDK | Anticipated | A repo-local Bun/TypeScript package (`packages/dacs-sdk/`) that collapses per-agent setup by implementing the cross-cutting machinery **once**: canonical form (JCS + NFC + decimals-as-strings, §A), domain-separated signing + content-hash chaining (§B.7), SR-2 anchoring (incl. CF-4 logical-address assembly), and produce/consume for the spine artifacts (IdentityBundle, Listing, AgreementDocument, SettlementEvidence, AttestationBundle, RatingRecord), plus recipe-registry resolution + execution for DACS-2 vetting. It should wrap `@kynesyslabs/demosdk` as the Demos rail/substrate adapter; DACS artifact semantics, validators, conformance-vector consumption, and version targeting stay owned by this repo. First package plan: [`claudedocs/dacs-sdk-implementation-plan.md`](./claudedocs/dacs-sdk-implementation-plan.md). |
