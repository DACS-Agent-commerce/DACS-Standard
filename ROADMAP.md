...
| `pay-x402` conformance vector | Candidate (near-complete) | `pay-x402` is **exercised end-to-end by the reference implementation** (`agent-commerce-demo`: a buyer-side x402 client signing EIP-3009/Permit2 and settling USDC on Base) and settles **on-chain** (chain-verifiable like the `evm` rail via `settlementTxHash`/`chainId`, §9.5.7) — it meets the live-path + reference-implementation bar. The only outstanding item for parity with the vectored rails is landing a **§14 conformance vector**. |
...
| Conformance-summary completeness (§7.9, §9.10) — editorial | Candidate; editorial | Role→rule-range summary tables omit families that carry MUSTs. **DACS-2 §7.9:** **PSP-1..5** (ParserSpec semantics), **WN-1..6** (verification warnings), **GOV-2/3** (anchoring-phase disclosure), **PA-1..3** (progressive anchoring). **DACS-4 §9.10:** no Orchestrator row for **RAV-R1..5** (rail-availability obligations), and **AMEND-1..4** only implicitly under "amendment chain following". A conformance implementer reading either summary as the checklist would miss them; decide the owning role per family and add rows (no rule-text change). Also tighten §7.5.1(a)'s retry-summary wording, which reads looser than the governing VP-R1/VP-R3. Surfaced by the DACS-2/DACS-4 readability reviews (2026-06-07/08); no normative change. |
...
| Item | Status | Notes |
|------|--------|-------|
| DACS-X (dispute / execution-verification) | Anticipated; prototype in progress | Dispute initiation, arbitrator credentialing via DACS-1/2, named-arbitrator transcript disclosure, superseding outcome bundles (§11.2.1). The accountability layer — makes the session record adjudicable; it does not hide session content. **This is the *reactive* half of "verifying a service was delivered": when a buyer disputes a paid-for-but-undelivered (or wrongly-delivered) service, DACS-X adjudicates and can supersede the outcome. The *proactive* half — holding payment until delivery is confirmed — is the `pay-evm-erc8183` escrow-with-delivery-gate rail (DACS-4, above); the two are complementary, and v0.1 has neither (settlement is evidence-based, not escrow-gated, §9.9). Objective proof of service delivery exists today only where the result is attestable via `deliver-attested-payload` (DACS-2 provenance), not for arbitrary service execution.** |
...
| `suspiciousPatternFlags` on ReputationDerivation + min-bundleCount gating advice (#101) | Candidate; design-issue first | Optional behavioural-Sybil signal for low-tier selectors (e.g. single-counterparty clustering, zero-variance outcomes). Flags themselves are fine as an **open vocabulary**; the normative-threshold and min-`bundleCount` gating parts need an RFC so the standard doesn't hard-code heuristics that belong in the implementation layer (§10.5). Pairs with `identityTier` (#103) — identity quality + behavioural quality as two orthogonal signals. |
...
| Item | Status | Notes |
|------|--------|-------|
| Modular restructure — Core + stage modules + Primer (v0.2 format) | In progress; lands at the v0.2 cut | Re-packages the single specification into a non-normative **Primer** (the read-to-understand entry point), a **CORE** (shared types, canonical form, signatures, session model, substrate requirements, governance), five self-contained **stage modules** (DACS-1…DACS-5), and a **Profile** manifest — so a reader or implementer adopts *Primer + Core + one module* instead of the whole document. **No normative rule-text change**; conformance citations move (e.g. a `§6.3.4` CF-4 cite becomes a Core cite). The document split is complete and validated; the remaining step is **hoisting the cross-cutting rule families into Core** — CF-2 / CF-3 (ClaimReference canonical form), CF-4 (logical-address encoding), CD-1 (canonical decimal), ST-1…ST-8 (session state machine) — which still live in a stage module though they are used by ≥2 (test: used across modules → Core; stage-specific → module). It is a *move*, not a rewrite. |
