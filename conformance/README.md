...
- **DACS-VERIFY-0003 — resolved.** An earlier §6.3.4 rule derived the native address as sha256 of the logical address (64 hex), which did not match Demos StorageProgram addressing. The rule was corrected to the write-input derivation with first40hex truncation; a listing anchored per the current rule has been verified on the Demos testnet to produce a 40-hex `stor-` address that resolves and content-verifies (see #237). The corresponding `dacs1-native-address` vector asserted the superseded behaviour and has been removed.
...
The published golden vectors (`dacs1-cci-lei-defect`, etc.) assert the observed behaviour, so the evidence is runnable.
