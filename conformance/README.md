...
- **CANDIDATE (8)** — PB-1..PB-3 payee-destination binding vectors for §9.5.1 pre-pay refusal / pause behaviour.
...
- `candidate`: 8 candidate vectors, §9.5.1 PB-1..PB-3 payee-destination binding — agreement-carried destination match/mismatch, tier-2 resolution mismatch, applicable-but-unresolvable tier-2 pause, no satisfiable tier refusal, SB-3 fallback separation, repeated pay phases keyed by `(railId, phaseIndex)`, and post-field omission refusal.
