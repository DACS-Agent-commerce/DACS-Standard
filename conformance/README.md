...
- **GOLDEN (177)** — byte-stable reference outputs plus steward-corrected regressions pinned by executable predicates. Counts: 7 canonicalize, 5 decimal, 5 signing, 2 artifact-reference-shape, 24 DACS-1, 2 addressing, 4 §10.4 bundle, 37 settlement, 36 verify, 24 vet, 19 negotiate, and 12 governance checks.
- **CANDIDATE (59)** — 28 `PayeeBoundAgreementDocument` / PB-1..PB-3 vectors, 14 historical reputation outputs whose one-copy inputs lack authoritative-absence context, and 17 output-only DACS-X dispute/disclosure expectations awaiting public signed inputs and normative artifact contracts.
...
- `candidate`: 59 candidate vectors — 28 §8.5/§8.6 `PayeeBoundAgreementDocument` artifact compatibility plus §9.5.1 PB-1..PB-3 payee-destination binding; 14 pre-guard-(iv) reputation outputs retained for regeneration after valid resolution context is available; and the 17 output-only DACS-X dispute/disclosure expectations described above.
