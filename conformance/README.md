...
- **GOLDEN (192)** — byte-stable reference outputs plus steward-corrected regressions pinned by executable predicates. Counts: 6 canonicalize, 5 decimal, 5 signing, 2 artifact-reference-shape, 24 DACS-1, 2 addressing, 4 §10.4 bundle, 17 dispute/disclosure, 36 settlement, 36 verify, 24 vet, 19 negotiate, and 12 governance checks.
- **CANDIDATE (43)** — 29 `PayeeBoundAgreementDocument` / PB-1..PB-3 vectors plus 14 historical reputation outputs whose one-copy inputs lack authoritative-absence context.
...
- `candidate`: 43 candidate vectors — 29 §8.5/§8.6 `PayeeBoundAgreementDocument` artifact compatibility plus §9.5.1 PB-1..PB-3 payee-destination binding — legacy/current reader behaviour, legacy payoutBindings rejection, phase/artifact mismatch, cross-domain signature rejection, exact payout coverage, destination match/mismatch, positive tier-2 controlled-claim binding, positive tier-1 `pay-dem` intrinsic binding, tier-2 resolution mismatch, tier-2 resolver error, applicable-but-unresolvable tier-2 pause with exact recorded cause, SB-3 fallback separation, repeated pay phases keyed by `(railId, phaseIndex)`, and the corrected RD-5 chain-ID mismatch case — plus 14 pre-guard-(iv) reputation outputs retained for regeneration after valid resolution context is available.
