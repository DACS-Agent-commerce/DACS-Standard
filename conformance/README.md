...
- `fixtures/settlement/settlement-ap2-reference.json` — a byte-stable public fixture generated and verified with the official AP2 Python SDK at the pinned commit. It carries the separate CheckoutMandate and PaymentMandate presentations, public JWKs, byte-exact transaction-ID projection, a non-secret provider-status projection, the exact signed DAHR transaction and containing Demos block, the `ap2-sr3` receipt references, and deterministic signed `pay-ap2` SettlementEvidence. `tests/test_ap2_reference_fixture.py` independently checks the merchant JWS, canonical ULID, session/amount binding, known-answer DAHR transaction/block identities, Ed25519 signatures against an independently pinned testnet validator roster, inclusion/finality, request/response commitment, evidence hash, and DACS signature; it also rejects an internally consistent attacker-selected block roster. The raw provider body remains private because it contains a test-mode client secret.
