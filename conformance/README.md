...
- `fixtures/settlement/settlement-ap2-reference.json` — a byte-stable public fixture generated and verified with the official AP2 Python SDK at the pinned commit. It carries the separate CheckoutMandate and PaymentMandate presentations, public JWKs, byte-exact transaction-ID projection, provider-status body/hash and AP2-1 metadata binding, AP2 receipt attestation, and deterministic signed `pay-ap2` SettlementEvidence. `tests/test_ap2_reference_fixture.py` independently checks the merchant JWS, DACS projections, raw response hash, session/amount binding, evidence hash, and DACS signature; the DACS SDK opt-in integration re-runs the full upstream AP2 verifier.
