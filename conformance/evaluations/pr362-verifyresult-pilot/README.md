...
Verify that fresh result with the portable harness:

```sh
PYTHON_BIN=/path/to/python3 ./run.sh verify-portable /path/to/DACS-Standard /path/to/new-output/results.json
```
...
- `runner/eval_pr362.py` is the portable adaptation. It requires explicit paths,
  protects existing outputs, and can verify the old record against the exact old
  harness digest.
...
or verifier faults make a run invalid and must not become a candidate score.
