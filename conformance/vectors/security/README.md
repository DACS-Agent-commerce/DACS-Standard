...
| [`presence-only-claim-requirement-v0.7.json`](presence-only-claim-requirement-v0.7.json) | DACS-1 §6.3.3 PCR-1..PCR-6; DACS-2 §7.7.1 | 31 | `error` / `fail` / `indeterminate` / `pass` |
...
Thirty-one candidate cases make `ClaimRequirement.verificationRequired: false`
...
the controlled-key versus existence-only-LEI selector boundary. The set adds no
