...
| [`payee-destination-binding-v0.1.json`](payee-destination-binding-v0.1.json) | DACS-4 §9.5.1 (PB-1..PB-3); DACS-3 §8.5 PayeeBoundAgreementDocument.terms.payoutBindings | 11 | `error` / `fail` / `indeterminate` / `pass` |
...
11 candidate vectors for the §9.5.1 payee-destination gate. They model the
...
the payment is refused before submit, the session pauses as `indeterminate`
when an applicable binding cannot be resolved, or a resolver/input `error`
surfaces without downgrade.
