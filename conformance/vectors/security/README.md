...
| [`settlement-finality-verification-v0.8.json`](settlement-finality-verification-v0.8.json) | DACS-4 v0.8 §9.7.0 FV-1..FV-10; DACS-5 v0.6 §10.4.3/§10.5.1 | 47 | `error` / `fail` / `indeterminate` / `pass` |
...
Forty-seven candidate cases execute consumer-verifiable settlement finality for
...
The HTLC cases carry four structurally separate authenticated observations—
source lock/claim and destination lock/reveal—with distinct transaction/event
proofs and exercise unavailability of every arm; no aggregate status label can
stand in for them.
