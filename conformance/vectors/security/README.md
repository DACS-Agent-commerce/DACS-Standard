...
identity/vetting (§7.3.2), VerifyResult acceptance (§7.12), rail-availability selection (§9.4.4), sealed-envelope bid admission (§8.4.3), channel-message replay (§8.3.3 + CH-6), and fee disclosure + disclosed-fee reconciliation (§8.5.3 FS-1..FS-5, §9.7.2 FR-1..FR-4). These complement the lifecycle vectors in the
