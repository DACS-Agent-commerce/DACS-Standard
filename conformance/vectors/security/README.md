...
| [`claim-requirement-qualification-v0.3.json`](claim-requirement-qualification-v0.3.json) | DACS-2 §7.7.1 CRQ-1..CRQ-4 | 15 | `error` / `fail` / `indeterminate` / `pass` |
...
15 candidate vectors for qualifying authenticated, resolved `VerifyResult`
...
optional constraints, wrong recipe version, the inclusive and exceeded age
...
DACS `VerifyResult` fields, not a new wire artifact. Parameter matching requires
every requested own key to be present and canonically equal; additional
extracted-data keys remain valid.

#### Vector schema

Each entry in `vectors[]` contains `name`, `expected` (§7.5.1 four-value
