...
- **DACS-3 Negotiate.** Three negotiation patterns: fixed-price (buyer accepts listed terms), RFQ (bounded multi-turn bilateral), sealed-envelope (multi-bidder procurement with cryptographic commit-reveal). All three produce one of four signed `AgreementArtifact` types. The frozen `AgreementDocument` and `PayeeBoundAgreementDocument` keep their existing non-payee/payee meanings under `commit-agreement` and `commit-payee-bound-agreement`. The additive `IdentityBoundAgreementDocument` and `IdentityBoundPayeeAgreementDocument` are selected only by `commit-identity-bound-agreement` and `commit-identity-bound-payee-agreement`; the latter retains every payee payout and replacement obligation.
...
**Negotiation that's actually private.** DACS-3's RFQ and sealed-envelope patterns run over L2PS subnets. Bids are private to channel members. The negotiation transcript stays in the subnet (with optional encrypted anchoring). Only the final agreement hash and its commitment record are publicly anchored by default. The buyer's first offer doesn't get front-run by every other bidder.
