...
- **DACS-2 Vet.** A recipe registry mapping claim schemes to verification-method families (nine methods in v0.4, covering W3C VC, OAuth, TLSNotary, consensus-backed proxy attestation, EVM RPC reads, fresh ACME-style domain control, persistent Demos GCR domain binding, and self-signed). Each method produces a uniform `VerifyResult` shape with a four-valued `decision` (`pass | fail | indeterminate | error`). An aggregation algorithm produces an overall decision against the listing's requirements, with explicit precedence (failures > errors > indeterminates) and a normative `availability` field on every recipe so consumers can tell a live attestation path from a mocked or operator-gated one.
- **DACS-3 Negotiate.** Fixed-price acceptance, bounded bilateral RFQ, and sealed-envelope demand/procurement produce one of five signed `AgreementArtifact` types. The frozen `AgreementDocument` and `PayeeBoundAgreementDocument` retain their existing non-payee/payee meanings. The two identity-bound artifacts are selected only by their matching identity commitment phases. A current complete sealed auction uses typed bidder anchors, authenticated current-finalized candidate-set enumeration, a reproducible lowest/highest-price receipt, and only the independent `SealedSelectionAgreementDocument` / `commit-selection-bound-agreement` path. Older readers reject types they cannot safely enforce.
- **DACS-4 Settle.** Seven payment phase types cover EVM ERC-20, Solana SPL, cross-chain HTLC, cross-chain liquidity-tank, AP2 (FIDO Alliance Agent Payments), x402 (Coinbase's HTTP 402 micropayment scheme), and native DEM. Three delivery phase types cover storage-program-anchored deliverables, signed entitlement records, and DACS-2-attested payloads. Payments produce `SettlementEvidence`; deliveries produce signed phase-bound `DeliveryEvidence`. Every rail carries the same `availability` field as recipes so orchestrators preflight-check before selecting.
- **#392 finality candidate.** The existing `SettlementEvidence` path remains unchanged. A builder explicitly selecting the unallocated finality contract uses `FinalityBoundSettlementEvidence` and the matching DACS-5 finality-bound bundle/pointer, authenticates the exact signed rail/agreement/session, and supplies proof bytes to a consumer that recomputes FV. The repository fixtures do not provide a live-chain proof codec; without one, return `indeterminate` rather than trusting confirmation summaries.
...
**Negotiation that's actually private.** DACS-3's RFQ and sealed-envelope coordination runs over L2PS subnets. Bids stay private before reveal and the general negotiation transcript stays in the subnet (with optional encrypted anchoring). Complete sealed auctions intentionally publish signed commit/reveal records after the respective deadlines so an authenticated candidate-set proof and selection receipt can make omission detectable. The resulting selection-bound agreement commits the exact reproducible winner before settlement.
