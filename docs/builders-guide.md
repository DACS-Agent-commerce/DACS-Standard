...
- **DACS-3 Negotiate.** Fixed-price acceptance, bounded bilateral RFQ, and sealed-envelope demand/procurement. A current sealed auction uses the complete profile: typed bidder anchors, authenticated current-finalized candidate-set enumeration, and a reproducible lowest/highest-price receipt. Agreements are structurally selected as legacy `AgreementDocument`, payout-bound `PayeeBoundAgreementDocument`, or complete-auction `SealedSelectionAgreementDocument`; an older reader rejects a type it cannot safely enforce.
