...
| `DACS-3` | `DACS-3`, `PROFILES`, `REGISTRIES`, `ROADMAP`, `MAPPINGS` | Keep both agreement artifact types, both agreement commitment phases, and minimal channel trust properties; move RFQ/sealed-envelope detail, pattern ladders, transcript policy, rollout notes, and mapping material when not universal. |
