...
def listing_fixture(job_id: str = "01K1DPA0000000000000000000") -> dict[str, Any]:
...
def agreement_document(job_id: str) -> dict[str, Any]:
    listing = listing_fixture(job_id)
    ...
    unsigned: dict[str, Any] = {
        "agreementVersion": "1", "jobId": job_id,
        "listingRef": {"listingId": listing["listingId"], "version": 1, "contentHash": listing_hash(listing)},
        "parties": [
            {"role": "buyer", "bundleHash": ref._identity_bundle_hash(buyer_bundle), "primaryClaim": CLAIMS["buyer"], "vetRecordRef": {"kind": "test-vet", "value": "buyer-vet"}},
            {"role": "seller", "bundleHash": ref._identity_bundle_hash(seller_bundle), "primaryClaim": CLAIMS["seller"], "vetRecordRef": {"kind": "test-vet", "value": "seller-vet"}},
        ],
        "terms": {
    ...
    unsigned["signatures"] = [
        {"party": CLAIMS[role], "algorithm": "ed25519", "value": ref.b64u(
            ref.ed25519_sign(SEEDS[role], ref._AGREEMENT_DOMAIN + digest.encode("ascii"))
        )}
...
def commitment_anchor_receipt(record: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "evidence": {"kind": "test-bft-proof", "value": "finalized-commitment"},
    }
...
def payment_phase_input(intent: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "rail": {"railId": intent["railId"], "asset": "DEM", "handler": "pay-dem"},
...
def authorization_authority(intent: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "railDefinition": artifact(
            "authenticated-pay-dem-rail", "network",
            {"railId": intent["railId"], "asset": "DEM", "handler": "pay-dem"},
        ),
        "sessionContext": {"jobId": intent["jobId"], "parties": [
            {"role": "buyer", "primaryClaim": CLAIMS["buyer"]},
            {"role": "seller", "primaryClaim": CLAIMS["seller"]},
            {"role": "orchestrator", "primaryClaim": CLAIMS["orchestrator"]},
        ]},
    }
...
def purchase_intent(
    ...
    agreement = agreement_document(job_id)
    commitment = finality_commitment(job_id, agreement)
    slot_key = {"networkId": "demos:testnet-atomic", "railId": "demos-native-dem", "jobId": job_id, "phaseIndex": 2}
    conflict = {
...
def slot_proof(key: dict[str, Any], state: dict[str, Any]) -> dict[str, Any]:
    value = {"proofVersion": "test-1", "key": key, "state": state, "blockId": "block-slot-900"}
    return ref.sign_embedded(
        value, CLAIMS["network"], SEEDS["network"], ref._LEDGER_TEST_DOMAIN
    )
...
def final_receipt(
    ...
    if intent["profile"] == "dacs-purchase-v1":
        ...
        slot_before = copy.deepcopy(slot_payload["expected"])
    else:
    ...
    if outcome == "committed":
        if intent["profile"] == "dacs-purchase-v1":
            slot_after = {
                "state": "settled", "generation": slot_before["generation"],
    ...
    if outcome == "rolled-back" and intent["profile"] == "dacs-purchase-v1":
        slot_after = {
            "state": "rolled-back", "generation": slot_before.get("generation", 0),
            "workId": ref.work_id(intent), "conflictDigest": slot_payload["conflictDigest"],
        }
...
def vector(
    ...
    if surface in {"authorization", "attempts", "receipt", "lifecycle", "projection", "profile", "slot", "settlement", "role-anchor", "audit", "limits"}:
...
def execution_vectors() -> list[dict[str, Any]]:
    ...
    attempt_a = {"attemptVersion": "1", "workId": wid, "attemptId": "attempt-a", "canonicalWorkBytes": canonical, "authorizations": attempt_auths, "nativeTransactionRef": tx_a, "lifecycleEvidence": included}
    attempt_b = {"attemptVersion": "1", "workId": wid, "attemptId": "attempt-b", "canonicalWorkBytes": canonical, "authorizations": attempt_auths, "nativeTransactionRef": tx_b}
    attempt_a_non = {**attempt_a, "lifecycleEvidence": nonincluded}
    attempt_b_replacement = {**attempt_b, "replacementFor": "attempt-a"}
    ...
    proof_limit = {
        **limits_base,
        "proofMaterial": proof_material_with_canonical_size(
    ...
    return [
        ...
        vector("aw-replacement-on-not-found", ["AW-41", "AW-42"], "attempts", {"intent": intent, "attempts": [{**attempt_a, "lifecycleEvidence": None, "observation": "not-found"}, attempt_b_replacement], "winningAttemptId": None, "businessEffectAttempts": [], "publicKeys": PUBLIC_KEYS}, "indeterminate", "Ordinary not-found cannot prove a prior attempt will never execute."),
        ...
        vector("aw-exact-replay-returns-winner", ["AW-45"], "attempts", {"intent": intent, "attempts": [attempt_a, {**attempt_b, "replayOf": "attempt-a", "returnedWinner": "attempt-a"}], "winningAttemptId": "attempt-a", "businessEffectAttempts": ["attempt-a"], "publicKeys": PUBLIC_KEYS}, "pass", "Exact replay returns the selected winner without execution."),
        ...
        vector("aw-lifecycle-noninclusion-proof", ["AW-59"], "lifecycle", {"attemptId": "attempt-a", "nativeTransactionRef": tx_a, "claimedState": "authoritative-non-inclusion", "evidence": nonincluded, "publicKeys": PUBLIC_KEYS}, "pass", "Authenticated lifecycle evidence, not a client observation, establishes non-inclusion."),
        vector("aw-lifecycle-native-ref-mismatch", ["AW-59", "AW-60"], "lifecycle", {"attemptId": "attempt-a", "nativeTransactionRef": tx_a, "claimedState": "included-committed", "evidence": wrong_native_evidence, "publicKeys": PUBLIC_KEYS}, "fail", "Lifecycle evidence must bind the exact native transaction reference being queried."),
        vector("aw-lifecycle-missing-proof", ["AW-59", "AW-60"], "lifecycle", {"attemptId": "attempt-a", "nativeTransactionRef": tx_a, "claimedState": "authoritative-non-inclusion", "publicKeys": PUBLIC_KEYS}, "indeterminate", "Missing non-inclusion evidence is indeterminate."),
        ...
        vector("aw-finality-proof-missing", ["AW-47", "AW-48", "AW-60"], "receipt", {"intent": intent, "receipt": missing_finality, "publicKeys": PUBLIC_KEYS}, "indeterminate", "Receipt fields without finality proof are indeterminate."),
        ...
        vector("aw-proof-byte-limit-equality", ["AW-71"], "limits", proof_limit, "pass", "Canonical proof material exactly equal to maxProofBytes is admitted.", boundary=True),
        vector("aw-proof-byte-limit-one-over", ["AW-71"], "limits", proof_over_limit, "fail", "Canonical proof material one byte over maxProofBytes is rejected before commitment.", boundary=True),
...
def settlement_authority(intent: dict[str, Any]) -> dict[str, Any]:
    agreement = (
        intent["operations"][2]["payload"]["artifact"]
        if intent["profile"] == "dacs-purchase-v1"
        else intent["operations"][0]["payload"]["receipt"]
    )
    if intent["profile"] == "dacs-completion-v1":
        # The finalized Purchase receipt identifies the Purchase Work; vectors
        # carry its actual agreement authority separately below when needed.
        record = finality_commitment(intent["jobId"], agreement_document(intent["jobId"]))
    else:
        record = intent["operations"][3]["payload"]["artifact"]
    return {
        "finalityCommitment": record,
        "commitmentReceipt": commitment_anchor_receipt(record),
        "sessionContext": {"jobId": intent["jobId"], "parties": [
            {"role": "buyer", "primaryClaim": CLAIMS["buyer"]},
            {"role": "seller", "primaryClaim": CLAIMS["seller"]},
...
def settlement_slot_vectors() -> list[dict[str, Any]]:
    ...
    retry_intent = purchase_intent(
        generation=1, expected_state="rolled-back",
    ...
    return [
        ...
        vector("aws-failure-proof-missing", ["AWS-20", "AWS-28"], "settlement", failure_missing_proof, "indeterminate", "A missing failure operation proof is indeterminate."),
