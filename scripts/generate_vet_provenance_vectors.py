...
The fixture contains genuine Ed25519 signatures and full-content hashes for
...
def make_authorization(
    *,
    ...
    return envelope(artifact, auth_logical(artifact), full_signed_content=True)
...
def make_composite(auth_envelope: dict, verify_ref: dict, candidate_seed: str) -> dict:
    ...
    return envelope(artifact, composite_logical(artifact), full_signed_content=True)
...
def resign_envelope(item: dict, seed_name: str, domain: str, logical: str | None = None) -> None:
    ...
    # VRA/PCR keep the explicit full-signed-content exception even in
    # discriminator-coercion negatives; legacy Listing/VerifyResult do not.
    refreshed = envelope(
        item["artifact"],
        logical,
        state=state,
        full_signed_content="counterpartyContext" in item["artifact"],
    )
    item.clear()
...
def build_vectors() -> list[dict]:
    ...
    wrong_envelope = envelope(
        ...
        full_signed_content=False,
    )
    auth_item.clear()
    auth_item.update(wrong_envelope)
    sync_surface_refs(case)
    add("authorization-preimage-hash-is-not-anchor-content-hash", "VPA-10; PVC-2", "The VRA signature-preimage hash differs from and cannot replace the full signed anchored-content hash.", case, "fail")
    ...
    sync_composite_surface_refs(case)
    add("composite-preimage-hash-is-not-anchor-content-hash", "PVC-6", "The PCR signature-preimage hash differs from and cannot replace the full signed anchored-content hash.", case, "fail")
