...
| HTLC-1..HTLC-10 | §9.5.4 | buyerSalt entropy/confidentiality/non-reuse; HKDF derivation + input-uniqueness; canonical claim order; per-chain hashlocks; timelock asymmetry on absolute expiry (pinned params, source-finality margin); HTLC-9/ST-8 asymmetric resolution with both signatures bound to independent expected phase-orchestrator authority and optional supersession signer continuity; exact safe-integer chain/time values; HTLC-10 free-option | `conformance/fixtures/settlement/htlc9-asymmetric.json`, `conformance/fixtures/settlement/htlc9-asymmetric-resolved.json` |
