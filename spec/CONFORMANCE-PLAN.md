...
| LAA-1..LAA-7 | §9.5.1 | fixed-address steward-signed activation checkpoint derived from the authenticated session/commitment substrate; authenticated absence only on that substrate; absence must cover the authenticated payment-effect head, never a caller-supplied position; checkpoint, commitment, and settlement-evidence receipts share one exact substrate/finality/genesis ordering domain before positions are compared; current pay-bearing payee-bound requirement (payee-bound or identity-bound-payee); zero-length in-flight transition; exact agreement/commitment/evidence hash plus job/session/phase binding and finalized receipt ordering; backdated producer timestamp ignored; same-position strict ordering; wrong-substrate and cross-order-domain refusal; missing/conflicting/pruned/reorged authority; four-value precedence with malformed/unhashable inputs as `error`; historical audit without current payment upgrade; zero-pay exclusion | `conformance/vectors/security/legacy-agreement-admission-v0.8.json`; `tests/test_legacy_agreement_admission_vectors.py` |
...
| RSV-1..RSV-4 + settlement-verified reputation derivation | §10.5.1 | preserve released `derivationVersion: "1"` and `replayableDerivationVersion: "1"` semantics; require distinct settlement-verified discriminators before applying exact cross-copy `settlementEvidence[]` reference-multiset agreement and independent SettlementEvidence admission; successful legacy-agreement payments must pass LAA historical-era proof; rejected/indeterminate evidence excludes the job without fault; empty presented set contributes no volume; settlement-verified replay re-verifies immutable/finalized authority or refuses the receipt; unsupported readers reject before type-specific action | `conformance/vectors/security/reputation-settlement-reference-divergence-v0.4.json`; `conformance/vectors/security/reputation-settlement-semantics-v0.4.json`; `conformance/vectors/security/legacy-agreement-admission-v0.8.json` |
