...
| SAC-1..SAC-10 | §8.4.4 | new-type refusal; exact signed bidder record/job/listing/phase/address authority; pinned candidate-set binding; authenticated latest-finalized complete prefix result; exact set hash/count/order and every-record accounting; unavailable fact blocks instead of excluding a bidder; CD-1 lowest/highest-price reproduction and SE-5 tie-break; first-acceptable/rule-ref refused before execution; exact receipt signature/ref/finality; winner/price/role/payout closure into the selection-bound agreement; omitted better reveal, stale set, fork, malformed record, and receipt substitution | `conformance/vectors/security/sealed-auction-completeness-v0.6.json`; `tests/test_sealed_auction_completeness_vectors.py` |
