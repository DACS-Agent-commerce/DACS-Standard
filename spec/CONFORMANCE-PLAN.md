...
| AWT-1..AWT-8 authenticated-window current profile | §10.5 | preserve all five older derivation shapes as historical/partial; require the distinct current-profile discriminator, job-bound replay, and RSV; reconcile before windowing; bind the exact selected bundle and expected/replacement transaction relation to an independently verified established/finalized SR-2 receipt; use only `blockRef.timestamp` with inclusive boundaries; replacement/reorg/conflict/missing/malformed/mismatched proof is indeterminate and non-countable without `finalisedAt` fallback; replay re-verifies concrete selected/history objects; legacy-era claims require a verified policy/producer/session/profile/commit/pre-current-revision projection. The linked corpus starts from the disclosed post-reconciliation authoritative-copy precondition and executes AWT-6 membership after it; two-copy reconcile-before-window ordering remains a normative implementation obligation not independently exercised by this set. | `conformance/vectors/security/reputation-authenticated-window-v0.6.json` |
