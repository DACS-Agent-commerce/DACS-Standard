...
| IBH-1..IBH-6 | CORE §B.2 / §8.5 / §9.5 / §10.4 | new-artifact-only bare IdentityBundleHash; closed current/base PhaseType admission; genuine presentation, signature-omitted artifact refs, CVR/VerifyResult signatures, exact signed-Listing requirement, pinned registry and §7.7.1 aggregate replay; verifier nonce and exact `vetRecordRef` joins; all agreement parties including sealed-envelope losing bidders; role/claim/job/every-hash-position checks; missing authority trichotomy; commit/payment/terminal refusal; 4×4 phase/artifact and domain replay matrices; independently pinned fixture-only receipt observer with native binding/ordering tamper cases (not live-substrate proof); payee payout/replacement inheritance; malformed nested-type totality; modeled base agreement-reader plus executable base terminal-reference refusal with provenance | `conformance/vectors/security/identity-bundle-hash-binding-v0.1.json`; `tests/test_identity_bundle_hash_binding_vectors.py` |
