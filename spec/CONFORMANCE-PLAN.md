...
| DPA-1..DPA-9 | §9.6.3 | missing listing method rejected before payment; distinct PayloadAttestationRecord discriminator/domain; exact payload-byte digest; job/agreement/DeliverableSpec/method binding; native proof and transaction verification; pass-only success gate; required current DeliveryEvidence (or narrow legacy SettlementEvidence) hash/anchor/ref closure; unresolved proof remains indeterminate; evidence signer cannot substitute; cross-session replay and cross-type coercion reject; DAHR UTF-8/hash/tx profile | `conformance/vectors/security/payload-attestation-binding-v0.1.json`; `conformance/vectors/security/phase-bound-delivery-evidence-v0.7.json` |
| PDE-1..PDE-8 | §9.7 / §10.4.3 | distinct DeliveryEvidence discriminator/domain; signed job/phase index and exact evidence address; phase-indexed deliverable/entitlement/payload-attestation addresses with independent renewal/attempt counters; conditional per-kind success closure and failure-without-closure; exact entitlement hash/signature and credential ref/cleartext/renewal binding with delivered-only semantics; fail/error/indeterminate separation; single-delivery legacy arm with DV-5 refusal; exact one-to-one DACS-5 mapping with missing/extra/pointer negatives and mixed payment+delivery membership | `conformance/vectors/security/phase-bound-delivery-evidence-v0.7.json` |
