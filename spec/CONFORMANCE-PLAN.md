...
| CA-1..CA-9 | §8.6 | refuse irreversible Settle effects until finalized receipt; double-commit reject across both record types; immutability after anchor; type-specific domain-separated commitment signature; reject coercion across the exact five-way agreement/phase matrix; orchestrator authority independent of SR-2 deployer/owner/address; agreement binding by verified party signatures + `agreementHash`; identity companions only on identity paths; selection receipt only on the selection path; finality record's signed `createdAt` distinct from receipt-derived `committedAt`; legacy `committedAt` historical-anchor cross-check; exact-one discriminator and anti-coercion; SIG-2 cross-domain replay refusal | `conformance/vectors/security/payee-destination-binding-v0.1.json`; `conformance/vectors/security/identity-bundle-hash-binding-v0.1.json`; `conformance/vectors/security/commitment-anchor-authority-v0.3.json`; `conformance/vectors/security/commitment-record-compatibility-v0.1.json`; `conformance/vectors/security/sr2-anchor-lifecycle-v0.1.json`; `conformance/vectors/security/sealed-auction-completeness-v0.6.json` |
...
| DPA-1..DPA-9 | §9.6.3 | missing listing method rejected before payment; distinct PayloadAttestationRecord selector/domain admitted only after authenticated phase/domain context; arbitrary exact payload-byte digest; job/agreement/DeliverableSpec/method binding; complete reference plus authoritative job/phase/method/attempt locator derivation; native proof and transaction verification; pass-only success gate; required current DeliveryEvidence (or narrow legacy SettlementEvidence) hash/anchor/ref closure; unresolved proof remains indeterminate; evidence signer cannot substitute; cross-session replay and cross-type coercion reject; DAHR exact-byte/hash/tx profile | `conformance/vectors/security/payload-attestation-binding-v0.1.json`; `conformance/vectors/security/phase-bound-delivery-evidence-v0.7.json` |
| PDE-1..PDE-8 | §9.7 / §10.4.3 | distinct DeliveryEvidence selector/domain checked after authenticated operation/domain family context; signed job/phase index and exact evidence address; phase-indexed deliverable/entitlement/payload-attestation addresses with independent renewal/attempt counters; conditional per-kind success closure and failure-without-closure; full-canonical-reference receipt maps with included/finalized and completed-finalized/independently-resolvable lifecycle gates for every inner dependency; exact entitlement hash/signature and credential ref/arbitrary exact bytes/renewal binding with delivered-only semantics; authenticated effective ACL/encryption-recipient/stored-ciphertext commitments; deterministic malformed collection/member handling with fail/error/indeterminate separation; single-delivery legacy arm with DV-5 refusal; exact one-to-one DACS-5 mapping with missing/extra/pointer negatives and mixed payment+delivery membership | `conformance/vectors/security/phase-bound-delivery-evidence-v0.7.json`; `tests/test_bundle_pointer_admission_delivery_authority.py` |
| PIPE-1..PIPE-5 | §9.9 | ≥1 deliver (pay-* optional, §6.3.4(8)); deterministic ordering; pay↔deliver gating; phase repetition with independent phase-bound payment/delivery evidence | `conformance/vectors/security/phase-bound-delivery-evidence-v0.7.json` |
| PIPE-6 | §9.9 | finalized, session-bound DACS-3 commitment receipt before payment or irreversible delivery; submitted/accepted/index-visible/unverified-included negatives | `conformance/vectors/security/sr2-anchor-lifecycle-v0.1.json` |
...
| Delivery phases | §9.6 | storage-program (normal + extended-pointer); phase-indexed entitlement sig/anchor/scope and exact credential delivery; attested-payload composing method-native DACS-2 evidence through the phase-indexed DACS-4 PayloadAttestationRecord and DPA gate | `conformance/fixtures/`; `conformance/vectors/security/payload-attestation-binding-v0.1.json`; `conformance/vectors/security/phase-bound-delivery-evidence-v0.7.json` |
...
| Bundle/pointer admission (333-P01) | §10.4 / CORE §11.1.2 | establish family from a fixed protocol/address operation or uniquely verified registered domain before selector/schema action; record members and caller labels are nonauthorizing; exactly one supported full-bundle selector/major; pointer never promoted through shared `bundleVersion`; supported selector plus inert unknown signed member accepted and hash-bound; future-only, dual-recognized, and unsupported-major refused; fixed FAB/EBFAB pointer entry points retain their domains; generic BundleExtendedPointer bytes/procedure preserved with its missing generic domain explicitly unresolved and no fallback; shared-address generic dispatch remains unsupported without independent operation authority | `tests/test_bundle_pointer_admission_delivery_authority.py`; regenerated signed fixture in `conformance/fixtures/evidence-bound-fault-bundle-compatibility-v0.4.json` |
| Bundle production | §10.4 | two-sided anchoring at role addresses; `anchoredByRole` ↔ address (mismatch rejected); authenticated pre-parse family context plus exact type-specific selectors, bundle domains, and FAB/EBFAB extended-pointer domains; legacy generic pointer procedure unchanged; v0.4 SEB claims use `EvidenceBoundFaultAttestationBundle`, while released types retain v0.3 semantics; completed bundles preserve the ST-11 finalized/resolvable dependency-and-bundle gate, including transitive delivery references; publish a signed BundleBinding per anchored copy on a write-input substrate (BB-1/BB-2), without treating BundleBinding as family authority | `conformance/fixtures/`; `conformance/fixtures/evidence-bound-fault-bundle-compatibility-v0.4.json`; `tests/test_bundle_pointer_admission_delivery_authority.py` |
| SEB-1..SEB-6 (`EvidenceBoundFaultAttestationBundle` typed evidence exact set) | §10.4.3 | derive `(phaseIndex, phaseKind)` from the verified signed ordinary listing pipeline or APR-7 independently recomputed effective pipeline and the domain-verified EBFAB `phaseSummary`, never caller input, raw `pay-alternative`, or optional pointers; establish payment/current-delivery evidence family from authenticated phase and uniquely verified signature domain before requiring its exclusive selector; execute mixed payment+delivery, repeated, successful, failed, aborted-before-result, and ST-8 invocations through the sequential SEB gate; reject unknown action-bearing delivery fields; require full-reference receipts and verified `included`/`finalized` evidence, including inner delivery dependencies, with the stricter ST-11 `finalized`/independently-resolvable gate for completed bundles; reject raw duplicates before set formation; require a duplicate-free bijection and exact optional-pointer agreement; deterministic rejection precedes unrelated uncertainty; preserve a valid EBFAB result across EBFAB/EBFAB, EBFAB/FAB, and EBFAB/legacy reconciliation | `conformance/vectors/security/bundle-settlement-evidence-bijection-v0.4.json`; `conformance/fixtures/evidence-bound-fault-bundle-compatibility-v0.4.json`; `conformance/vectors/security/phase-bound-delivery-evidence-v0.7.json`; `conformance/vectors/security/alternative-payment-projection-v0.1.json`; `tests/test_bundle_pointer_admission_delivery_authority.py` |
...
| RSV-1..RSV-4 + settlement-verified reputation derivation | §10.5.1 | preserve released `derivationVersion: "1"` and `replayableDerivationVersion: "1"` semantics; require distinct settlement-verified discriminators before applying exact cross-copy `settlementEvidence[]` reference-multiset agreement and independent type-dispatched payment/delivery evidence admission; rejected/indeterminate evidence excludes the job without fault; only verified successful payment `SettlementEvidence` contributes volume; empty presented set contributes no volume; settlement-verified replay re-verifies immutable/finalized authority or refuses the receipt; unsupported readers reject before type-specific action | `conformance/vectors/security/reputation-settlement-reference-divergence-v0.4.json`; `conformance/vectors/security/reputation-settlement-semantics-v0.4.json` |
| Determinism receipt | §10.5.3 | current public query admission authenticates expected party/window/basis even for an empty result; ordered verifier-owned content-reference authority fixes each entry's job/role/participant before callbacks; current paths require authenticated crypto keys; `bundleRefs` = `reconciled` set, ascending-`contentHash` order; `resolutionContext` one entry per ref; re-derive byte-identical `metrics`/`bundleCount`; named legacy replay remains frozen | `conformance/` |
