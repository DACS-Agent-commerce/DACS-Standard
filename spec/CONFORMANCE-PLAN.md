...
| HTLC-1..HTLC-10 | §9.5.4 | buyerSalt entropy/confidentiality/non-reuse; HKDF derivation + input-uniqueness; canonical claim order; per-chain hashlocks; timelock asymmetry on absolute expiry (pinned params, source-finality margin); HTLC-9/ST-8 asymmetric resolution with signer equality to independently expected phase-orchestrator authority; recursive duplicate-JSON-member refusal before canonicalization/signature verification; HTLC-10 free-option | `conformance/fixtures/settlement/htlc9-asymmetric.json`, `conformance/fixtures/settlement/htlc9-asymmetric-resolved.json` |
