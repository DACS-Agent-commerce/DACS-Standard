...
| Bundle consumption | §10.4.3 | two-sided lookup; one-sided only after authoritative absence; unqualified `not found`, transport failure, or inconsistent state views → indeterminate; independent second copy restores normal unified/divergent classification; divergence = `outcome`/`phaseSummary` contradiction (advisory skew is NOT divergence); DACS-5 reputation excludes divergent and indeterminate jobIds | `conformance/fixtures/`; `conformance/vectors/security/bundle-absence-evidence-v0.3.json` |
