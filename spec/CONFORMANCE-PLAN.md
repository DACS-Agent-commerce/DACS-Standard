...
| VPA-1..VPA-10, PVC-1..PVC-6, PVPC-1..PVPC-11 | §7.7.3, §7.7.4, §7.8.3 | bilateral positive plus authenticated historical DACS-1 v0.7 / DACS-3 v0.5 two-bidder procurement replay; explicit current-profile refusal of the historical sealed-phase/commit pair; signed roster hash/count and exact `2 × |C|` preauthorization barrier; independently verified required-claim evidence; distinct requirement bodies/hashes; exact publisher/counterparty/verifier identities; full-body/ref/session/registry/pinned-Listing provenance closure; logical/native/receipt tuple and explicit preimage-vs-full-content hash separation; missing/duplicate/swapped/wrong-party/cross-context provenance; legacy/new discriminator and domain separation; canonical output/audit ordering with non-positional keyed inputs; conclusive exclusion vs retryable-error admission; exact winner/loser/publisher agreement mapping; accepted→finalized audit-dependency projection; explicit pre-winner and multi-fault DACS-5 gates. The historical procurement case does not certify SAC-complete selection for a current DACS-3 v0.6 session. The set excludes sealed-demand and the PVPC-6 scoped malformed-counterparty terminal-error carve-out. It assumes baseline DACS-1 Listing/PhaseStep schema validation, independently rechecks the provenance-relevant Listing major/version/ID/validity/phase/pricing gates, and does not claim a complete signed outer AttestationBundle fixture. | `conformance/vectors/security/vet-provenance-v0.6.json` + `tests/test_vet_provenance_vectors.py` |
...
| Rules | Home | Exercise (intent) | Vectors |
| --- | --- | --- | --- |
| ST-1..ST-11 (state machine) | §10.3.1 | every `(from→to)` legal-only; illegal-pair reject (ST-1); abort/timeout only in action-pending states before irreversible effects (ST-3/ST-9), never rate/audit pending; rate branch + non-fatal (ST-4/5); ST-7 pause→resume / →failed-substrate including audit-pending; ST-8 `settle-asymmetric` forward-resolution, non-terminal; ST-10 cancellation; ST-11 `audit-pending` requires finalized/resolvable dependencies and bundle before `finalised`, recursively including provenanced Vet authorization→listing/signature/body/verifier closure; terminal→`outcome` map (ST-6) | `conformance/fixtures/settlement/`; `conformance/vectors/security/sr2-anchor-lifecycle-v0.1.json`; `conformance/vectors/security/vet-provenance-v0.6.json` |
