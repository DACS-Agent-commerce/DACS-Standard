...
| AP2-1..AP2-7 | §9.5.6 | AP2-3 split-credential registration eligibility; AP2-6 byte-exact provider idempotency key, NFC, job/phase separation, and malformed input; AP2-7 first binding, exact-tuple retry/resume without a second payment, cross-job/cross-phase replay refusal, and conflicting-store error; upstream checkout-JWT non-deterministic-signature policy | `conformance/vectors/security/ap2-handler-safety-v0.5.json` |
