...
| LAB-1..LAB-7 (legacy bundle era admission) | §10.4.1 / §10.5.1 | governed per-substrate checkpoint signature/address/finality; on Demos, signed `LegacyBundleCheckpointBinding` discovery from the steward well-known index or DACS catalog, exact logical/native/hash/anchor/writer/nonce binding, real checkpoint and binding signatures, and finalized receipt order; receipt-order authority instead of producer timestamps; current FAB/EBFAB production; exact-hash/exact-role pre-checkpoint legacy proof; same-role historical replay without byte/type rewriting; buyer↔seller role rebind and fresh/backdated/post-checkpoint legacy exclusion; missing/conflicting/pruned/reorged/unorderable authority indeterminate without fault; admission before BB-6/reconciliation/absence/metrics; replay retains era evidence | `conformance/vectors/security/legacy-bundle-admission-v0.6.json` |
...
| Released reputation derivation v1 | §10.5.1 | preserve DACS-5 v0.3 `derivationVersion: "1"` and `replayableDerivationVersion: "1"` metric semantics: base §10.4.3 divergence only, no RSV admission, Agreement-backed completed-session volume; all outcome partitions, denominator/null rules, two-sided reconciliation, guard (iv), ratings, ordering, and windowing remain unchanged. LAB is the current-use input trust gate: an older receipt without qualifying legacy-era evidence is reproducible only as historical/partial, not current-action-bearing reputation | `conformance/fixtures/reputation/`; `conformance/fixtures/session-bundles-reputation.json`; `conformance/vectors/security/bundle-absence-evidence-v0.3.json`; `conformance/vectors/security/receipt-rederivation-v0.3.json`; `conformance/vectors/security/legacy-bundle-admission-v0.6.json` |
