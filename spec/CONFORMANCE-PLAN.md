...
| IBH-1..IBH-6 | CORE §B.2 / §8.5 / §9.5 / §10.4 | exact bare-lowercase current IdentityBundle digest across DACS-2/3/4/5; wrong digest and encoding refusal; prefix mutation never normalised; authenticated legacy agreement projection preserves signed bytes and emits a bare terminal value; role/claim/resolved-bundle binding | `conformance/vectors/security/identity-bundle-hash-binding-v0.1.json`; `tests/test_identity_bundle_hash_binding_vectors.py` |
