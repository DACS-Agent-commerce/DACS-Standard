...
| RAV-1..RAV-7, RAV-R1..RAV-R5 | §7.4.5, §9.4.4 | recipe availability consumer+steward behaviour; rail preflight; no disabled/failed new-session selection; no mocked production-session selection; live-pinned in-flight continuation after a later disabled revision; RAV-R5 authoritative signed read with discovery hints ignored for protocol decisions | `conformance/vectors/security/rail-availability-selection-v0.1.json` + `tests/test_rail_availability_selection_vectors.py` |
