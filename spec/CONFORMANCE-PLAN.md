...
| IBH-1..IBH-6 | CORE §B.2 / §8.5 / §9.5 / §10.4 | new-artifact-only bare IdentityBundleHash; genuine presentation and CVR signature verification; verifier nonce and exact `vetRecordRef` joins; role/claim/job/every-hash-position checks; missing authority trichotomy; commit/payment/terminal refusal; 4×4 phase/artifact and domain replay matrices; payee payout/replacement inheritance; malformed nested-type totality; modeled base agreement-reader plus executable base terminal-reference refusal with provenance | `conformance/vectors/security/identity-bundle-hash-binding-v0.1.json`; `tests/test_identity_bundle_hash_binding_vectors.py` |
