...
| CR-1 (`demos-gcr-domain` control binding) | §6.3.2 | controlled use (`presentedBy` / reputation keying) requires the presentation to verify under the authenticated `gcrRecord.account` — directly, or via an authenticated, **presentation-bound** SR-1 linkage demonstrating key possession; an unrelated presenter still satisfies a required claim but stays valid-but-uncontrolled; an SR-1 linkage not bound to this presentation (bearer/ambient) or bound to a different presentation (replay) MUST NOT confer control | `conformance/fixtures/identity/domain-claim-bundle-semantics.json` |
...
| GE-1..GE-10, GF-1..GF-3 (`demos-gcr-domain`) | §7.3.10 | authenticated GCR evidence binding — context / substrate / inclusion-covers-`sourceTx` / finality / SB-2 write-authorization relation (account-key AND node-key realisations; a write authorized by a non-bound account fails) / record.account==claimAccount / record.host==identifier / `recordRef.contentHash`==hash(record) self-consistency / record.proofUrl==derive(identifier) / **GE-10 provenance** — the record is the state `sourceTx` actually wrote (a coherent record the tx did not write → fail; provenance unrecoverable → indeterminate); freshness derived from `recordedAt` with the anti-reissue gate, the effective-window bound, and **GF-3** recordedAt future-dating/positive-window sanity within `recordedAtSkewToleranceSec`; unavailability (not-found / transport-error / record-unreadable / unverifiable inclusion / unfinalized) → indeterminate; SB-1 (inclusion/finality/state-recovery) / SB-2 / SB-3 substrate hooks | `conformance/fixtures/identity/domain-claim-bundle-semantics.json` |
