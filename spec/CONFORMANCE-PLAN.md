...
| SB-1..SB-3 | §9.5.8 | complete PC-2 `(jobId, CF-4 railId, phaseIndex)` anchor tuple matches signed evidence plus authenticated agreement/phase context before projection; signed `evm-event` / `solana-instruction` / `x402-event` coordinate independently matches ledger settlement; deterministic event-key projection; legacy exactly-one-match replay and ambiguous/unavailable `indeterminate`; unsigned index ignored; discriminator stripping/cross-type replay rejected; cross-tuple collision groups select only an exact finalized settlement-side binding, otherwise every competitor is non-countable `indeterminate`, with producer/hash/arrival/SR-2 order inert and later discovery revoking provisional counts (SB-2); authenticated rail policy selects required-binding versus explicitly unbound posture; required verified match continues, mismatch fails, absent/unavailable/pruned/reorganised is non-countable `indeterminate`, malformed is `error`, and caller/legacy downgrade hints are inert; for `pay-x402` EIP-3009, byte-exact `SHA-256(UTF8("dacs-sb3:v1:") || UTF8(NFC(jobId)) || 0x3a || ASCII(decimal(phaseIndex)))`, NFC, and retry/no-random-substitution behavior (SB-3) | `conformance/vectors/security/settlement-event-identity-v0.6.json`; historical key corpus `conformance/vectors/security/sb2-settlement-uniqueness-v0.1.json`; `conformance/vectors/security/sb2-collision-authority-v0.8.json`; `conformance/vectors/security/sb3-eip3009-nonce-v0.1.json`; `conformance/vectors/security/sb3-binding-required-v0.8.json` |
