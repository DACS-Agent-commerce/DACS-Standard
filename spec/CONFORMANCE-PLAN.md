...
| RSC-1..RSC-9 (current revocation-state completeness) | §6.3.4 | require the signed Listing's stable state line; authenticate the latest finalized native value; verify head type/domain/current key, checkpoint-to-head sequence/hash/receipt continuity, append-only sparse-Merkle transitions, exact tuple inclusion/non-membership, revoked>indeterminate>absent precedence, and replay context; stale/censored/forked/unavailable authority never establishes active | `conformance/vectors/security/revocation-state-completeness-v0.8.json` |
