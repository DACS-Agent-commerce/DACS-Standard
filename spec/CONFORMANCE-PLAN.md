...
| APR-1..APR-8 | §9.9.1 | exactly one listing-only choice slot with at least two complete distinct refs and no concrete pay sibling; same-snapshot authenticated resolution to supported non-recursive handlers; DEM/x402 full-ref selection in either array order; deterministic index-preserving projection; exact payee-bound payout key; concrete evidence/bundle kind; legacy-reader refusal; pre-signature reselection only; fresh-job post-signature switch; post-authorization and indeterminate retry make zero second-rail authorization calls; ordinary PIPE-5 repetition unchanged | `conformance/vectors/security/alternative-payment-projection-v0.1.json`; `tests/test_alternative_payment_projection_vectors.py` |
