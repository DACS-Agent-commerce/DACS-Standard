...
| SPA-1..SPA-8 signed participation and rating admission | §10.3.2 / §10.5 | require the alleged absent obligor's target-signed exact job/listing/roster/active-phase/action/deadline admission plus authoritative absence before current one-sided blame; compare the admitted prefix with the `(index, kind, outcome)` projection of authenticated bundle entries while permitting their evidence members; allow no-admission self-blame only for an absolute-fault bundle whose verified signer set contains its hashed `faultedParty`, never for legacy role-relative blame; verify commit/settle agreement binding, fresh nonce, exact timeout marker, finalized admission-before-deadline and bundle-at/after-deadline receipts in one clock domain; exclude missing/rejected/indeterminate evidence without fault; require fully signed completed bundle, authenticated successful rate phase, exact ref, and exact target role for current ordinary ratings; replay rejects omitted, substituted, or unused conditional context | `conformance/vectors/security/reputation-participation-admission-v0.7.json` |
...
| RT-1, RT-2 (rate phase) | §10.6.1 | run-after-settle; one-record-per-direction; rating domain-sep sig; RT-1 producer-reject out-of-range/over-length; RT-2 deriver-exclude non-conforming; `dimensions` opaque; current-profile lifecycle/target admission is additionally SPA-7 | `conformance/vectors/security/reputation-participation-admission-v0.7.json` |
