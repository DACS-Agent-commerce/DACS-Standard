...
| PS-1..PS-3 | §8.8 | exactly-one negotiate phase; exactly one of the four agreement commitment phases immediately follows; pattern ↔ pricing-model compatibility | `conformance/` |
| Agreement validation | §8.5.2 | price-band / rail-acceptance / deliverable / deadline / pattern checks; exact four-way artifact ↔ signed commitment-phase match; exact pay-phase payout-binding coverage; `priceAnchor` valid-when-present, optional | `conformance/` |
| Agreement-artifact compatibility | §8.5, CORE §11.1.2 / §11.2.5 | historical reader accepts its two released agreement artifacts and refuses both identity-bound types; current reader applies four exclusive discriminators/domains/phases; both historical artifacts retain byte-exact validity under both commitment forms | `conformance/vectors/security/payee-destination-binding-v0.1.json`; `conformance/vectors/security/identity-bundle-hash-binding-v0.1.json` |
| IBH-1..IBH-6 | CORE §B.2 / §8.5 / §9.5 / §10.4 | new-artifact-only bare IdentityBundleHash; closed current/base PhaseType admission and mandatory negotiate→commit ordering; genuine presentation with a distinct bounded one-use verifier challenge consumed on failure and exact authenticated admission retention/reuse; signature-omitted artifact refs, CVR/VerifyResult signatures, exact signed-Listing requirement, pinned registry and §7.7.1 aggregate replay; exact `vetRecordRef`, session roster, role/claim/job/digest/paying-key joins; all agreement parties including sealed-envelope losing bidders; missing authority trichotomy; common commit/payment/terminal refusal; 4×4 phase/artifact and domain replay matrices; genuine APR projection with exact slot, precommit payout and signed/finalized disposition including cannot-settle proof; complete EBFAB/SEB payment+delivery set and finalized agreement/CVR/commitment/bundle dependency joins; independently pinned fixture-only native observer (not a production codec or live-substrate proof); historical byte preservation and explicit non-authorizing unmodeled-stage result; malformed nested-type totality | `conformance/vectors/security/identity-bundle-hash-binding-v0.1.json`; `tests/test_identity_bundle_hash_binding_vectors.py` |
