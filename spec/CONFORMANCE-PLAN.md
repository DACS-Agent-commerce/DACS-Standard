...
| AWT-1..AWT-8 authenticated-window current profile | §10.5 | preserve all five older derivation shapes as historical/partial; require the distinct current-profile discriminator, job-bound replay, and RSV; reconcile before windowing; bind the exact selected bundle to an independently verified established/finalized SR-2 receipt; use only `blockRef.timestamp` with inclusive boundaries; replacement/reorg/conflict/missing/mismatched proof is indeterminate and non-countable without `finalisedAt` fallback; replay re-verifies the exact proof; legacy-era claims require authenticated profile-revision evidence | `conformance/vectors/security/reputation-authenticated-window-v0.6.json` |
