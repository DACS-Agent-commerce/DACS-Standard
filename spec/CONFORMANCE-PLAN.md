...
| XN-1..XN-11 | §9.4.3 / §9.5.7 | discriminated provider-neutral definition; no global/operator configuration; complete signed ref equality and duplicate/first-ID refusal; local capability separation; v1/v2 exact challenge matching; method/URL/body/redirect, amount, payTo, payer, `extra`, and extensions substitution refusal before authorization; whole-object challenge/receipt NFC+JCS hashes; numeric-version generic evidence; adapter-canonical cross-rail SB-1 key; independent settlement/finality floor; unsupported schemes and acknowledgement downgrade; disabled legacy default; retry reconciliation without reauthorization | `conformance/vectors/security/x402-negotiated-protocol-v0.7.json` + `tests/test_x402_negotiated_protocol_vectors.py` |
