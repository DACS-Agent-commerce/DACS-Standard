...
| PB-1..PB-3 | §9.5.1 | agreement-carried destination matches the phase tuple (PB-1); mismatched destination aborts before payment; strongest applicable identity-binding tier decides; applicable-but-unresolvable tier 2 pauses without tier-3 downgrade; no satisfiable tier refuses; SB-3 fallback is not imported into the pre-pay gate; repeated pay phases use distinct `(railId, phaseIndex)` bindings | `conformance/vectors/security/payee-destination-binding-v0.1.json` |
