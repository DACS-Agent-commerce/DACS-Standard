...
| Agreement validation | §8.5.2 | price-band / complete-reference JCS rail selection (never first matching railId or parameter merge) / deliverable / deadline / pattern checks; `x402:protocol` requires the payee-bound artifact; artifact ↔ commitment-phase match; exact pay-phase payout-binding coverage; `priceAnchor` valid-when-present, optional | `conformance/vectors/security/x402-negotiated-protocol-v0.7.json`; `conformance/` |
...
| PB-1..PB-3 | §9.5.1 | payee-bound destination match/mismatch; exact `(railId, phaseIndex)` coverage; tier-1/2/3 selection including tier-1 intrinsic and tier-2 controlled-claim positive cases; EVM tier-2 applicability derived by exact CAIP-2 chain equality from a static rail or complete agreement-signed `x402:protocol` selection, never a later challenge/receipt or a pre-filled flag/name alias; applicable-unresolvable pause with exact recorded cause; resolver `error` stays `error`; no downgrade; SB-3 fallback is not imported into the pre-pay gate; legacy agreement follows legacy behaviour and carries no PB claim | `conformance/vectors/security/payee-destination-binding-v0.1.json`; `conformance/vectors/security/cci-xm-rail-chain-applicability-v0.5.json`; `conformance/vectors/security/x402-negotiated-protocol-v0.7.json` |
| XN-1..XN-11 | §9.4.3 / §9.5.7 | discriminated provider-neutral definition; no global/operator configuration; complete signed ref equality and duplicate/first-ID refusal; local capability separation; v1/v2 exact challenge matching; method/URL/body/redirect, bounded public-target classes, amount, payTo, payer, `extra`, and extensions substitution refusal before an instrumented wallet-call site; presentation-verified buyer bundle pinned by hash plus adapter-native EVM/Solana paying-key-to-address proofs and attacker-address refusal; whole-object challenge/receipt NFC+JCS hashes; numeric-version generic evidence; adapter-canonical cross-rail SB-1 key including both non-canonical EVM spellings; exact finality binding profile; protocol-defined unsupported-scheme dispositions and acknowledgement downgrade; disabled legacy default; retained `(jobId, phaseIndex, requirement, authorization, transaction)` reconciliation with zero retry authorization calls | `conformance/vectors/security/x402-negotiated-protocol-v0.7.json` + `tests/test_x402_negotiated_protocol_vectors.py` |
