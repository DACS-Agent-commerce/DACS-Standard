...
| Bundle consumption | §10.4.3 | two-sided lookup; one-sided → aborted-by-self; divergence = bundle `outcome`, shared-index `phaseSummary.kind`/`outcome`/`errorClass`, or entry-presence contradiction (advisory skew is NOT divergence); out-of-band dispute handling; "disputed" is a consumer verdict, not an outcome value; DACS-5 reputation always excludes the jobId | `conformance/fixtures/`; `conformance/vectors/security/phase-kind-divergence-v0.3.json` |
