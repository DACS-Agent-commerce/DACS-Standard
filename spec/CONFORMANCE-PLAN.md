...
| --- | --- | --- | --- |
| Claim-reference parser | §6.3.1 | every scheme: valid canonical / valid non-canonical (canonicalise only at the defined read boundary) / invalid signed grammar (reject without repair) / unknown-scheme (not silently accepted); exact two-hex-digit DID escapes with method-specific bytes retained; strict `did:demos:agent:<64hex>` profile; CF-2 parameters and CF-3 identity kept distinct; EVM `cci-xm` PB profile requires lowercase `evm`, a canonical positive-decimal chain ID within the signed-JSON safe-integer range, and a non-empty otherwise-opaque address, maps only that chain ID to `eip155:<chainId>`, keeps larger textual identifiers readable but PB-2-inapplicable, ignores ClaimReference parameters for chain selection, and never guesses `mainnet` / `testnet` / `sepolia` | `conformance/vectors/`; shared affected-path checks in `tests/test_dacs1_vet_golden_inputs.py`; `conformance/vectors/security/cci-xm-rail-chain-applicability-v0.5.json` |
...
| BR-1..BR-5 (bundle reader) | §6.3.2 | accept-conformant; reject unsigned / missing-required-`verifiedBy`; selector requires exact-claim control plus verified-fresh or explicit presence authorization; unknown-scheme → unverified; Demos agent DID scheme-case and identifier-canonicalisation cases; SIWD `dacs:<hex>` Resource + session-`Nonce` match | `conformance/fixtures/identity/`; `conformance/vectors/security/presence-only-claim-requirement-v0.7.json`; `conformance/fixtures/identity/dacs1-vet-golden-inputs-v0.1.json` |
| PCR-1..PCR-6 + match() (BundleRequirement) | §6.3.3 | presence required/missing/expired/JCS-parameter match from signed claim metadata; `issuedAt` informational; optional passing/failing/stale/unavailable `verifiedBy` non-deciding for presence; malformed ref and presence `maxAge`/`recipeVersion` error; verified predicates use authenticated result method/data only; verified member cannot use bare presence; mixed required/oneOf; exact key control vs existence-only LEI; same-scheme predicate/selector laundering rejection | `conformance/vectors/security/presence-only-claim-requirement-v0.7.json`; `conformance/fixtures/identity/dacs1-vet-golden-inputs-v0.1.json` |
| Vet canonical/time boundary | §7.7.1 / CORE §B.2 | strict repository JCS for hashes and predicate equality (`1` equals `1.0`; non-finite/out-of-profile numbers and invalid Unicode reject); direct entrypoints validate containers without throwing and preserve four dispositions/CRQ precedence; signed Composite `generatedAt` supplies historical decision time, is no later than trusted execution/receipt time and no earlier than participating results, while unsigned wrapper `evaluatedAt` is ignored | `tests/test_dacs1_vet_golden_inputs.py`; `tests/test_claim_requirement_qualification_vectors.py`; `conformance/fixtures/identity/dacs1-vet-golden-inputs-v0.1.json` |
...
| VPC-1..VPC-5, MA-1..MA-3 | §7.8, §6.3.3 | phase order / two-sided; verifier-owned active `vet-pending` admission pins phase/attempt, actor/evaluated party, expected verifier role/identity, independently authorized orchestrator and anchor writer, recipe registry, trusted time and issuer-owned consume-on-attempt nonce ledger; verifierIdentity/composite/ref signer equality without deriving authority from presented identity; independently resolved logical/native/content/writer receipt at durable-acceptance minimum before reversible return and finalized for terminal replay; no broadcast-ack promotion; idempotent same-address reconciliation to finality; fail-or-indeterminate; matching + controlled exact `presentedBy` | `conformance/`; `conformance/vectors/security/presence-only-claim-requirement-v0.7.json`; `conformance/fixtures/identity/dacs1-vet-golden-inputs-v0.1.json` |
