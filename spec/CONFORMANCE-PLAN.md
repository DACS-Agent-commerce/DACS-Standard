...
| PDE-1..PDE-8 | §9.7 / §10.4.3 | distinct DeliveryEvidence discriminator/domain; signed phase index and exact evidence address; phase-indexed deliverable/entitlement/payload-attestation addresses with independent renewal/attempt counters; per-kind success closure; exact credential ref/cleartext/renewal binding with delivered-only semantics; fail/error/indeterminate separation; single-delivery legacy arm; exact one-to-one DACS-5 mapping with optional per-phase pointers absent/present | `conformance/vectors/security/phase-bound-delivery-evidence-v0.7.json` |
