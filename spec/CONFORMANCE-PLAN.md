...
| PRA-1..PRA-5 (parser applicability) | §7.4.1 / §7.6 | all nine methods classified exactly once; parser-consuming recipe requires ParserSpec; new native-only recipe omits it; historical signed native-only field remains readable but inert; mixed recipe applies it only on a parsing selection; native decision/data cannot be overridden; invalid shape fails before invocation | `conformance/vectors/security/recipe-parser-applicability-v0.5.json` |
