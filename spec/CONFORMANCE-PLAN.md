...
| PB-1..PB-3 | §9.5.1 | payee-bound destination match/mismatch; exact `(railId, phaseIndex)` coverage; tier-1/2/3 selection; applicable-unresolvable pause; no downgrade; no satisfiable tier refuses; legacy agreement follows legacy behaviour and carries no PB claim | `conformance/vectors/security/` |
...
| Bundle consumption | §10.4.3 | two-sided lookup; one-sided → aborted-by-self; divergence = `outcome`/`phaseSummary` contradiction (advisory skew is NOT divergence); out-of-band dispute handling; "disputed" is a consumer verdict, not an outcome value; DACS-5 reputation always excludes the jobId | `conformance/fixtures/` |
