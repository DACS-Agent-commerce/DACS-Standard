...
| SPA-1..SPA-8 signed participation and rating admission | §10.3.2 / §10.5 | require the alleged absent obligor's target-signed exact job/listing/roster/active-phase/action/deadline admission plus authoritative absence before current one-sided blame; verify effective-pipeline prefix, commit/settle agreement binding, fresh nonce, exact timeout marker, finalized admission-before-deadline and bundle-at/after-deadline receipts in one clock domain; exclude missing/rejected/indeterminate evidence without fault; require fully signed completed bundle, authenticated successful rate phase, exact ref, and exact target role for current ordinary ratings; replay rejects omitted, substituted, or unused conditional context | `conformance/vectors/security/reputation-participation-admission-v0.7.json` |
