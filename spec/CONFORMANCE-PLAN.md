...
| LAA-1..LAA-7 | §9.5.1 | fixed-address steward-signed activation checkpoint derived from the authenticated session/commitment substrate; authenticated absence only on that substrate; checkpoint, commitment, and settlement-evidence receipts share one exact substrate/finality/genesis ordering domain before positions are compared; current pay-bearing payee-bound requirement; zero-length in-flight transition; exact agreement/commitment/evidence hash and finalized receipt ordering; backdated producer timestamp ignored; same-position strict ordering; wrong-substrate and cross-order-domain refusal; missing/conflicting/pruned/reorged authority; four-value precedence; historical audit without current payment upgrade; zero-pay exclusion | `conformance/vectors/security/legacy-agreement-admission-v0.8.json`; `tests/test_legacy_agreement_admission_vectors.py` |
