...
| LRR-1..LRR-6 (listing-time rail resolution) | §6.3.4 | pay-phase↔accepted-ref binding; duplicate canonical-ref rejection while permitting distinct same-ID selections; canonical registry and definition resolution; exact rail/version/hash/signature/handler binding; conclusive unknown/mismatch rejection; unavailable authority indeterminate; PA-2/PA-3 no in-code fallback; session-start re-resolution | `conformance/vectors/security/listing-rail-registry-resolution-v0.4.json` |
