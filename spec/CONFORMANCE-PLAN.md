...
| AWP-1..AWP-21 (Atomic Purchase and Completion profiles) | §8.6.1 | exact closed operation sets, order, and required roles; complete already-signed Vet artifacts and finalized resolvable assertions accepted while live or nondeterministic Vet actions reject; authenticated pipeline eligibility is exactly `pay-dem` for Purchase and `deliver-storage-program` for Completion, with distinct payment and delivery phase indexes; agreement and §8.5.2 checks complete before payment; consensus block time controls deadline edges; commitment, slot mutation, payment, and critical writes share one rollback boundary and one authenticated BFT-final Work receipt; only a Work satisfying every co-finality condition receives the profile-scoped alternative to CA-1/SR2-8/PIPE-6, and any missing guarantee restores those sequential gates before signing or submission; Completion binds the finalized Purchase receipt, establishes the commitment operation's projected finalized `AnchorReceipt` before delivery, and binds the exact delivery, but does not itself finalize DACS-4 evidence or DACS-5 bundles; post-Purchase failure, refund, correction, and delivery guarantees remain separate authorized flows | `conformance/vectors/security/atomic-work-purchase-completion-v0.1.json` |
...
| AWS-1..AWS-15 (Atomic pay-dem payment slot) | §9.5.10 | proof-bound, component-wise and type-strict `(networkId, railId, jobId, phaseIndex)` identity; one consensus compare-and-set ledger across competing Works before any payment effect; domain-separated conflict digest binds agreement, commitment, payer, payee, asset, and CD-1 amount; changed digest rejects before payment; exact in-flight replay reconciles and settled replay returns the authoritative receipt; rollback-bound generation advance retains the conflict digest and binds the prior failure receipt; `indeterminate` holds the slot; consensus slot and Work ledgers, never SDK orchestration state, control uniqueness and winner selection | `conformance/vectors/security/atomic-work-settlement-slot-v0.1.json` |
| AWS-16..AWS-29 (Atomic Work settlement evidence) | §9.7.3 | distinct version selected before interpretation, both/neither discriminator rejection, and no coercion to legacy `SettlementEvidence` or `demos:{txHash}`; independently resolve the distinct Atomic Work receipt reference, compare its commitment and complete-envelope hash, reconstruct its operation root, and verify the exact structured operation proof; reference, proof subject, committed leaf, signed network/session/rail/phase context, parsed Atomic payment/delivery address, and proof-bound slot tuple match component-wise; classify mismatch / malformed input / unavailable proof as `fail` / `error` / `indeterminate`; pay and delivery success evidence copies only proven operation outputs; the closed network-scoped `atomicSettlementId` feeds SB-2; orchestrator signature cannot replace receipt, slot, finality, or artifact proof; per-generation audit publication is create-only or byte-identical catch-up, binds the complete signed evidence bytes at the derived address under its own finalized CORE §5.1 receipt, and never resubmits payment; client, submitter, SDK, Indexer, and classifier claims never establish coherence | `conformance/vectors/security/atomic-work-settlement-slot-v0.1.json` |
...
| SEB-1..SEB-6 (`EvidenceBoundFaultAttestationBundle` SettlementEvidence exact set) | §10.4.3 | derive `(phaseIndex, phaseKind)` from the verified signed listing pipeline and the domain-verified EBFAB `phaseSummary`, never caller input or optional pointers; cover repeated, successful, failed, aborted-before-result, and ST-8 invocations; require verified `included`/`finalized` evidence and the stricter ST-11 `finalized`/independently-resolvable gate for completed bundles; reject raw duplicates before set formation; require a duplicate-free bijection and exact optional-pointer agreement; deterministic rejection precedes unrelated uncertainty; preserve a valid EBFAB result across EBFAB/EBFAB, EBFAB/FAB, and EBFAB/legacy reconciliation | `conformance/vectors/security/bundle-settlement-evidence-bijection-v0.4.json`; `conformance/fixtures/evidence-bound-fault-bundle-compatibility-v0.4.json` |
...
| AWB-1..AWB-10 (Atomic Work role anchoring and audit tail) | §10.4.2 | Completion receipt alone cannot satisfy ST-11; current v1 admits no DACS-5 bundle-anchor operation and uses the ordinary audit tail; independently verify both Work IDs, winners, finality, ordered leaves, and inclusion paths; if a later versioned profile admits a Work-carried bundle copy, exact role authorization must match the agreement and bundle party map independently of outer submitter, fee payer, native signer/writer, or deployer; that copy retains ordinary bundle signatures, `anchoredByRole`, BB-1..BB-8, and same-role `BundleBinding.signer`; the proof chain matches binding, native address, content hash, projected anchor, operation leaf/path, Work receipt, and winner component-wise; missing proof is `indeterminate`, contradictory proof rejects, and neither Indexer nor SDK status substitutes; audit-tail replay is byte-identical and lifecycle-reconciled; repair never recreates Purchase payment or silently uses the legacy path; `audit-pending` remains until every ST-11 and AWB check succeeds | `conformance/vectors/security/atomic-work-audit-role-v0.1.json` |
...
For substrates other than Demos that claim conformance, additional capability
tests apply. The Atomic DACS Work category additionally applies to every
binding, including Demos, that claims the optional CORE §5.2 profile:
...
- **Atomic DACS Work (AW-1..AW-77).** Verify authenticated capability gating, pre-signing fallback, the two-business-Work plus audit-tail scope, and no silent dual path; pure-data JCS/NFC/safe-integer canonicalization, role/operation ordering, complete-byte `workId` derivation, unknown-member preservation, identity conflicts, consensus-time expiry boundaries, execution-profile pinning, and refusal of payloads lacking a selected byte-exact execution-profile schema; closed critical operation kinds, graph and reference validation, shared overlay, and static external-effect refusal; complete role authorizations with domain, algorithm, signer, session, slot, and operation replay resistance independent of the outer submitter; byte-identical replacement attempts, authenticated replacement authority, one consensus winner, and idempotent replay; independently verified finality, non-circular receipt commitments, ordered operation-receipt Merkle roots including empty/odd trees, rollback/state/non-membership proofs, fee/nonce separation, false-absence refusal, and crash-boundary recovery; independently verifiable `demos-work-operation-v1` projection to every `AnchorReceipt`; and node-enforced limits, structured-identity comparison, authoritative evidence, and preservation of the last authenticated state under `indeterminate`. The candidate corpus MUST label every case's acceptance, rejection, indeterminate, or malformed disposition. Positive, negative, and boundary applicability is evaluated independently for each rule: every applicable cell MUST have at least one semantically meaningful fixture, while a non-applicable cell MUST be machine-readable and carry a non-empty rationale. A true boundary is limited to a quantitative/cardinality edge or a decision edge between lifecycle, discriminator, proof-availability, or version/profile states; an `indeterminate` or malformed result is not inherently boundary coverage, and an ordinary signature/hash pass-fail pair MUST NOT be padded with a boundary marker. A multi-rule vector MUST attribute a boundary only to the specific rule IDs whose edge it exercises. The gate MUST fail on any uncovered applicable cell. Before this profile may claim conformance, it also requires mutation of every signed or hashed field and failure injection at every critical operation position; grouped `ruleRefs` are not a substitute. Vectors: `conformance/vectors/security/atomic-work-identity-v0.1.json` (AW-1..AW-29 plus AW-76..AW-77), `conformance/vectors/security/atomic-work-authorization-v0.1.json` (AW-30..AW-38), and `conformance/vectors/security/atomic-work-execution-recovery-v0.1.json` (AW-39..AW-75).
