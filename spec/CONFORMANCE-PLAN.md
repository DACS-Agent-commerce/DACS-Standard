...
| Reserved combined current-use derivation (#391+#392) | §10.5.1 | discriminator reserved; no passing output until authenticated historical-era/role admission, checkpoint ordering, exact job-bound replay, and finality-bound current evidence are all integrated; no boolean legacy substitute and no existing-type relabel | `tests/test_settlement_finality_verification_vectors.py` (FV interface and explicit gate only; combined execution pending #391) |
