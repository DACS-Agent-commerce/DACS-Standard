...
| PS-1..PS-3 | §8.8 | exactly-one negotiate phase; exactly one of the three agreement commitment phases immediately follows; complete sealed phase ↔ selection-bound commitment pairing; pattern ↔ pricing-model compatibility | `conformance/` |
...
| Agreement-artifact minor compatibility | §8.5, CORE §11.1.2 / §11.2.5 | legacy reader accepts AgreementDocument and structurally rejects payout- or selection-bound types before action; current reader selects exactly one of all three discriminators/domains; complete sealed phase pairs only with the selection-bound commitment; reject multiple/neither/unknown discriminators and cross-domain signatures | `conformance/vectors/security/payee-destination-binding-v0.1.json`; `conformance/vectors/security/sealed-auction-completeness-v0.6.json` |
| CA-1..CA-9 | §8.6 | refuse irreversible Settle effects until finalized receipt; double-commit reject across both record types; immutability after anchor; type-specific domain-separated commitment signature; reject coercion among all three agreement/commitment phase types; orchestrator authority independent of SR-2 deployer/owner/address; agreement binding by verified party signatures + `agreementHash`; finality record's signed `createdAt` distinct from receipt-derived `committedAt`; legacy `committedAt` historical-anchor cross-check; exact-one discriminator and anti-coercion; SIG-2 cross-domain replay refusal | `conformance/vectors/security/payee-destination-binding-v0.1.json`; `conformance/vectors/security/commitment-anchor-authority-v0.3.json`; `conformance/vectors/security/commitment-record-compatibility-v0.1.json`; `conformance/vectors/security/sr2-anchor-lifecycle-v0.1.json`; `conformance/vectors/security/sealed-auction-completeness-v0.6.json` |
