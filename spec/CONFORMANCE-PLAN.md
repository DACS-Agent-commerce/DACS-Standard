...
| Bundle production | §10.4 | two-sided anchoring at role addresses; `anchoredByRole` ↔ address (mismatch rejected); canonical-equality happy path (excludes anchoredByRole/signatures); type-specific bundle and extended-pointer domains; v0.4 SEB claims use `EvidenceBoundFaultAttestationBundle`, while released bundle types retain v0.3 semantics; publish a signed BundleBinding per anchored copy on a write-input substrate (BB-1/BB-2) | `conformance/fixtures/` |
| SEB-1..SEB-6 (`EvidenceBoundFaultAttestationBundle` SettlementEvidence exact set) | §10.4.3 | given explicit authenticated durable evidence-bearing `(phaseIndex, phaseKind)` keys for invocations that ran to a success or failure outcome, and independently resolved job-bound reference keys, reject raw duplicates before set formation; resolved ST-8 uses only the success successor while expired ST-8 uses the standing interim failure; require duplicate-free bijection including distinct-reference alias rejection; omitted per-phase pointers pass, present pointers agree uniquely and stay inside the top-level array; deterministic structural/pointer rejection precedes unrelated uncertainty. The candidate set pins this set/mapping/precedence surface; SEB-1 authority derivation and SB-1 job/index recovery require separate implementation fixtures before golden promotion | `conformance/vectors/security/bundle-settlement-evidence-bijection-v0.4.json` |
