...
| Rules | Home | Exercise (intent) | Vectors |
| --- | --- | --- | --- |
| RD-1..RD-7 | §9.4.3 | steward-sig + domain separator; anchor; version monotonicity; railType↔asset/network consistency, including identical positive `asset.chainId` / `network.chainId` for EVM assets; same-railId phase-handler invariance; complete model-matching consumer finality profile before finality-bound production | `conformance/fixtures/settlement/`; `conformance/vectors/security/listing-rail-registry-resolution-v0.4.json`; `conformance/vectors/security/cci-xm-rail-chain-applicability-v0.5.json`; `conformance/vectors/security/settlement-finality-verification-v0.8.json` |
| PC-1..PC-7 | §9.5.1 | input-shape; anchored evidence; `attestationRef`/`anchorReceipt` deferrable after independently verified rail finality under PC-7; all `errorClass` values; PC-5 currency-resolution; PC-6 current success uses distinct finality-bound evidence with producer report present/absent-on-delivery; PC-7 all-rail evidence catch-up without payment resubmission | `conformance/fixtures/settlement/`; `conformance/vectors/security/sr2-anchor-lifecycle-v0.1.json`; `conformance/vectors/security/settlement-finality-verification-v0.8.json` |
...
| FV-1..FV-10 | §9.7.0 | exclusive finality-bound type/domain and exact signed rail-profile binding; producer report cannot select model/strength; raw proof shape; network/genesis/provider identity; transaction/receipt/log/instruction inclusion; authenticated head and ancestry; independent depth/commitment/BFT quorum computation; stale fork/reorg/replacement/head conflict/freshness; HTLC carries four independent source-lock/source-claim/destination-lock/destination-reveal observations plus explicit cross-leg relations (aggregate status forbidden); tank composite and provisional provider capture; four-value precedence; DACS-5 RSV reuse | `conformance/vectors/security/settlement-finality-verification-v0.8.json`; `tests/test_settlement_finality_verification_vectors.py` |
...
| RSV-1..RSV-4 + settlement-verified reputation derivation | §10.5.1 | preserve released `derivationVersion: "1"` and `replayableDerivationVersion: "1"` semantics; require distinct settlement-verified discriminators before applying exact cross-copy `settlementEvidence[]` reference-multiset agreement and independent evidence admission; finality-bound current success reuses DACS-4 FV-1..FV-10; rejected/indeterminate/error evidence excludes the job without fault; empty presented set contributes no volume; replay re-verifies immutable/finalized authority or refuses the receipt; unsupported readers reject before type-specific action | `conformance/vectors/security/reputation-settlement-reference-divergence-v0.4.json`; `conformance/vectors/security/reputation-settlement-semantics-v0.4.json`; `conformance/vectors/security/settlement-finality-verification-v0.8.json` |
