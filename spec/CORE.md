...
> Draft — **DACS Core v0.3** (on the first-public-release DACS v0.1 baseline). v0.3 defines portable logical-to-native resolution and the non-recursive registry bootstrap, adds mandatory raw JSON admission before canonicalisation, hashing, or signature verification, and registers the DACS-4 finality-bound settlement-evidence, finality-observation-response, legacy-agreement activation-checkpoint, legacy-payment-reservation, and legacy-transition-evidence signature domains; it is a declared **breaking pre-v1 corrective candidate** under §11.1.2, pins one byte-exact canonical `jobId` grammar across the stack, and corrects the shared SN-4/Vet execution boundary so presentation admission uses verifier-issued, issuer-owned nonce state and authenticated current-profile context rather than caller-projected authority. The Vet correction leaves the existing signed Vet artifact shapes and domains unchanged, but a pre-correction implementation carrying the same v0.3 label is not presumed interoperable. v0.2 defined the normative SR-2 write lifecycle, portable anchor receipts, and cross-stage anchoring gates. See [CHANGELOG](../CHANGELOG.md) for normative change history.
...
Binding-specific proof and reconciliation metadata MUST be carried by the
portable `evidence: {kind, value}` member, not added as binding-specific
top-level `AnchorReceipt` fields. A binding MAY canonically encode structured
proof inside `evidence.value` (including authenticated native order,
replacement causality, or lineage completeness) and MUST define how a consumer
decodes and verifies it. Those facts have authority only through that verified
binding evidence; an unverified sidecar or producer assertion cannot supply
them.
...
- **Numeric safe-magnitude constraint.** Every JSON number in a signed or content-hashed DACS document MUST be finite, admitted by a correctly rounded IEEE-754 binary64 JSON-number conversion without overflow or non-zero underflow, and have mathematical magnitude no greater than 2^53−1 (9,007,199,254,740,991) as established from the raw number token before conversion. Both integral and fractional numbers are permitted within that bound and MUST be serialised exactly as RFC 8785 §3.2.2.3 specifies; in particular, negative zero serialises as `0`, fixed notation is used for magnitudes from 10^−6 (inclusive) to 10^21 (exclusive), and scientific notation is used outside that interval. NaN and positive or negative Infinity are not JSON numbers and MUST be rejected. Any quantity that may exceed the DACS bound (token IDs, uint256 values, large on-chain counters or block numbers) MUST be carried as a decimal string — or, where ABI conventions apply, a `0x`-prefixed hex string — rather than a bare JSON number. Producers MUST NOT emit, and readers MUST reject, a signed or content-hashed document containing a JSON number outside this profile.

  > **Note (non-normative).** The magnitude bound is a DACS interoperability profile layered on JCS, not a prohibition on fractions. It prevents values whose intended integer identity cannot be reproduced by every binary64 JSON parser. Monetary amounts remain CD-1 decimal strings; fractional JSON numbers are used by bounded measurements and metrics such as completion rates and ratings.

- **Raw JSON admission (rule CF-5).** Before RFC 8785 canonicalisation, content-hash computation, signature verification, schema validation, or any security decision, a consumer of an externally supplied signed or content-hashed DACS JSON document MUST validate the received UTF-8 JSON text without discarding raw-token information. This admission step MUST complete before an object-model canonicaliser is invoked and MUST:
  1. reject ill-formed UTF-8, a byte-order mark, malformed JSON syntax, parser extensions such as comments or `NaN`/positive or negative `Infinity`, and trailing non-whitespace data after the single JSON value;
  2. reject a duplicate decoded object member name at every nesting level, including names whose JSON escape spellings differ but decode to the same sequence of Unicode scalar values; member names that differ only by NFC/NFD spelling remain distinct under CF-1 and RFC 8785;
  3. reject a string value or member name containing a lone UTF-16 surrogate or any other sequence that cannot be represented as well-formed UTF-8 Unicode scalar values; and
  4. validate each raw number token against the numeric safe-magnitude constraint above before conversion can round or collapse it. The mathematical tokens `9007199254740992`, `-9007199254740992`, larger positive/negative values, and equivalent exponent spellings are outside the profile even if a host parser would round them to a nearby binary64 value. Valid JSON spellings that denote the same admitted binary64 value (for example `1e0`, `1E+0`, or negative zero) need not retain their lexical spelling and proceed to ordinary JCS serialisation; and
  5. reject JSON whose container nesting depth exceeds **128** before recursive parsing, tree admission, or canonical serialisation can depend on a host stack limit. A top-level object or array has depth 1, a top-level scalar has depth 0, and each object/array nested inside another container adds 1. Depth 128 is admitted; depth 129 is a DACS-profile failure with code `JSON-NESTING-TOO-DEEP`. Implementations MUST enforce this profile bound independently of their parser recursion setting and MUST NOT leak a stack/recursion exception as a protocol verdict.

  A consumer MUST NOT first decode or parse an external document into text or a lossy map/number object model and then claim CF-5 conformance: malformed received bytes, duplicate members, and pre-rounding numeric identity can no longer be reconstructed. An external-admission API MUST accept the exact received byte sequence, not an ordinary decoded string. A host API MAY pass a pre-parsed value only when it also supplies an authenticated, implementation-enforced guarantee that the exact received bytes already passed this rule; a caller assertion, decoded string, or re-serialised object is insufficient. A producer constructing an artifact locally MUST apply the same profile before signing or hashing it.

  Parse failure, DACS-profile admission failure, and later canonical-serialisation failure are distinct conformance outcomes. None is a failed cryptographic signature, and no rejected input has canonical DACS bytes, a DACS content hash, or signature authority. The executable raw-byte corpus is `conformance/vectors/security/raw-json-profile-v0.1.json`.

- **Unicode normalisation (rule CF-1).** Before computing the canonical form, every JSON string **value** in a signed or content-hashed DACS document MUST be Unicode-normalised to NFC. CF-1 applies only to values: object member names MUST remain exactly as received and MUST be sorted by their unescaped UTF-16 code units under RFC 8785 §3.2.3. Producers and verifiers MUST NOT NFC-normalise member names during canonicalisation; canonically equivalent NFC and NFD member-name spellings remain distinct members rather than colliding. Both producers and verifiers MUST apply values-only NFC at this stage, so the canonical form, content hash, and signatures are reproducible across implementations regardless of a value's precomposed/decomposed form. This lifts the per-field NFC requirement on `Identifier` (§B.1, CF-2) into a single normative pre-hash step covering the whole signed scope.
...
| DACS-5 finality-bound evidence fault attestation bundle | "dacs-finality-bound-evidence-fault-bundle:v1:" | §10.4.1 |
...
| DACS-5 FinalityBoundEvidenceFaultAttestationBundle extended pointer | "dacs-finality-bound-evidence-fault-bundle-pointer:v1:" | §10.4.2 |
...
**Non-signature hash-domain tags.** The table above registers *signature* domain separators (SIG-1 scopes to signatures). Five further `dacs-*:v1:` tags domain-separate normative hashes that are not signature payloads:

- `dacs-sealed-bid:v1:` — the historical sealed-envelope commitment preimage `sha256("dacs-sealed-bid:v1:" || sha256(canonical_JCS(bid)) || salt)` (§8.4.3; frozen);
- `dacs-sealed-bid-context:v1:` — the complete-profile context-bound sealed commitment preimage `sha256("dacs-sealed-bid-context:v1:" || sha256(canonical_JCS(SealedBidCommitmentContext)) || salt)` (§8.4.4 SAC-11); distinct from, and never interchangeable with, the historical tag;
...
All five follow the same domain-separation discipline, preventing cross-use of the resulting hashes. None is a signature `signed_bytes`, so SIG-1 and the "sign every artifact kind" conformance do not apply to them; they are the sanctioned non-signature hash-domain tags in v0.1.
...
- (SN-1) **Generator.** The verifier MUST generate the session nonce; a presenter-supplied nonce MUST NOT be trusted as the session binding. (A bundle or `SessionParticipationAdmission` MAY carry a `sessionNonce` the signer copied from the verifier's challenge — what SN-1 forbids is the verifier accepting a nonce it did not itself issue for this session. No artifact's `fresh`/`reused` assertion substitutes for the verifier's issued/consumed record.)
...
assert same-version interoperability with a pre-correction implementation:
only the future exact coordinated release tag or immutable specification
commit plus the complete tuple can establish the live profile. A Vet decision,
composite, invocation record, or conformance result produced or interpreted
only under pre-correction execution semantics remains historical evidence and
cannot establish current admission, authorize current protocol action, or be
relabelled as evidence for this correction. This does not revoke VP-C1..VP-C3
reuse of a structurally unchanged `VerifyResult` v1: after current-profile
admission, a current verifier may qualify that result from its authenticated
recipe family, version, signed predicates, times, and current trusted context.
The reusable result does not itself assert or prove a producing profile.
