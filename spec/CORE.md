...
**Breaking pre-v1 correction — PA-2 registry pin.** This v0.3 corrective-profile boundary replaces the existing PA-2 numeric-version-only behavior with the REQUIRED exact `(registryVersion, registryDescriptorHash)` pair. It is admitted only under the exact CORE/DACS-1/DACS-2/DACS-3/DACS-4/DACS-5 tuple `0.3/0.8/0.6/0.6/0.8/0.6` declared in §11.1.2 and [PROFILE](PROFILE.md); numeric-only historical inputs remain replayable but non-authorizing for new current-profile action.
...
CORE v0.3 together with the complete current tuple recorded in `PROFILE.md`
declares this boundary for two existing behaviours. First, the former “ULID or
substrate-equivalent” allowance, major-only listing admission, and
normalization-tolerant job-specific derivations are replaced by JID-1..JID-4.
Second, a numeric registry version alone no longer authorizes PA-2 registry
resolution or replay: current session and existing bundle inputs must carry and
authenticate the exact adjacent `registryDescriptorHash`, while historical
numeric-only artifacts remain readable but non-authorizing. Both corrections
require exact corrective-profile admission. Implementations MUST authenticate
the complete current tuple and exact release pin; no earlier composition,
numeric-only PA-2 pin, or pre-JID-1 profile implies live compatibility.
