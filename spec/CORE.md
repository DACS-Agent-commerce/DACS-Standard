...
- **Raw JSON admission (rule CF-5).** Before RFC 8785 canonicalisation, content-hash computation, signature verification, schema validation, or any security decision, a consumer of an externally supplied signed or content-hashed DACS JSON document MUST validate the received UTF-8 JSON text without discarding raw-token information. This admission step MUST complete before an object-model canonicaliser is invoked and MUST:
  ...
  A consumer MUST NOT first parse an external document into a lossy map/number object model and then claim CF-5 conformance: duplicate members and pre-rounding numeric identity can no longer be reconstructed from that value. A host API MAY pass a pre-parsed value only when it also supplies an authenticated, implementation-enforced guarantee that the exact received bytes already passed this rule; a caller assertion or a re-serialised object is insufficient. A producer constructing an artifact locally MUST apply the same profile before signing or hashing it.
