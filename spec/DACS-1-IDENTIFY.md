...
**Semantic-claim deduplication and `oneOf` consumption (normative — new in v0.6).** The matching algorithm above keys on the claim's **effective** reference (§6.3.2 canonical claim-reference resolution). Two claims that resolve to the same CF-3 canonical identity `(Scheme, Identifier)` — for one hostname, a `domain:<host>` claim and a legacy `web2:domain:<host>` claim both resolve to `domain:<host>` — represent **one semantic claim**, not two. Readers MUST apply:

1. **Identity equivalence** — claims sharing a canonical effective identity are one party/claim. A bundle carrying both alias forms for a hostname MUST NOT gain a second claim, a higher tier, or an extra `oneOf` satisfaction from the duplication.
...
4. **`oneOf` consumption is ordered and greedy** — `oneOf` groups are evaluated in declared order. A semantic claim (identified by its canonical effective reference) is consumed by the **first** group it satisfies and MUST NOT also satisfy a later group. So a single hostname presented as two alias forms satisfies at most one `oneOf` group, never two; two **distinct** hostnames may satisfy two groups normally.
5. **Producer rule** — a producer MUST NOT emit both the canonical `domain:<host>` and the legacy `web2:domain:<host>` form for one hostname in a bundle; a bundle carrying both is non-conforming (readers still resolve it per rule 1 for audit/replay compatibility).
