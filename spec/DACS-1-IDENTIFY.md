...
**Stage:** Identify (1st of 5). **Status:** Draft — **DACS-1 v0.4** on the common DACS v0.1 baseline. v0.4 requires a listing anchor to reach the CORE §5.1 finalized and independently resolvable gate before active discovery, and makes accepted-rail resolvability an executed canonical-registry check under LRR-1..LRR-6 rather than a self-referential listing assertion. v0.3 adds the §6.3.2 step (6) control gate, `pre-commit` `cancellationPolicy` handling §6, the sealed-envelope procurement listing-role clarification, the minor-safe `commit-payee-bound-agreement` phase, the §6.3.5/§6.3.6 DACS-5 bundle-binding discovery surfaces, and independently resolvable `RevocationBinding` revocation markers. **Depends on:** SR-1 (optional), SR-2 (required); composes with ERC-8004, W3C DIDs, A2A. **Used by:** DACS-2..5.
...
8. if pipeline contains any pay-* phase, `acceptedRails` MUST be present and non-empty and listing-time rail resolution under LRR-1..LRR-6 MUST return `verified`; if pipeline contains no pay-* phase, `acceptedRails` MAY be absent (the intake-only listing pattern — RFP intake, reverse auctions where the bid is the commitment, free services gated by reputation, sealed-bid procurements settled out-of-band);
...
- (LRR-1) **Unambiguous listing binding.** Every `pay-*` phase MUST carry a string `parameters.rail`, and every such value MUST equal the `railId` of at least one `acceptedRails` entry. The raw `acceptedRails` array MUST NOT contain duplicate full-canonical `PaymentRailRef` values. Canonically distinct references MAY share a `railId` (the ID dispatches to one registered handler; the complete reference carries a selectable requirement), and every such reference is validated independently. Every entry in `acceptedRails`, including an entry not selected by a particular pay phase, is subject to LRR-2 through LRR-5. Mere membership in the listing's own array never establishes resolution.
- (LRR-2) **Authoritative source.** The reader MUST resolve one internally consistent rail-registry snapshot through DACS-4 §9.4.3. Under PA-2 or PA-3 this means reading `dacs4:registry:v0.1`, verifying the applicable steward/governance authority, and obtaining verified CORE §5.1 `finalized` receipts plus independent content resolution for the index and each selected rail definition. The reader MUST NOT use a catalog row, listing field, counterparty copy, or unauthenticated cache as registry authority.
- (LRR-3) **Reference-to-definition match.** For every `PaymentRailRef`, the authenticated index MUST contain that exact `railId`. If `railVersion` is present, the indexed and resolved definition MUST carry that exact version; otherwise the reader uses the index's latest version in the snapshot for this validation attempt. The fetched `RailDefinition` MUST match the index's anchor/content hash, verify under `dacs-rail:v1:`, and repeat the selected `railId` and `railVersion`. Its schema and RD-1..RD-5 requirements MUST validate.
- (LRR-4) **Phase-handler binding.** For every `pay-*` phase, the resolved definition selected by its `parameters.rail` MUST have `phaseHandler` equal to that phase's `kind`. A listing cannot route `pay-x402`, for example, through a definition registered for `pay-evm-erc20`.
- (LRR-5) **Disposition and precedence.** Return `rejected` when the listing binding is malformed or ambiguous under LRR-1, when an authenticated registry snapshot conclusively lacks the named rail/version, or when an otherwise valid resolved definition contradicts the listing or phase handler under LRR-3/LRR-4. Return `indeterminate` when the registry index, required definition, finality receipt, independent content, or steward/governance key is absent, unavailable, internally inconsistent, or cannot be authenticated. An unavailable authority does not make the listing valid and does not prove the publisher made a false claim. Return `verified` only after every applicable check succeeds. `rejected` takes precedence over unrelated resolution uncertainty that cannot alter the established contradiction.
- (LRR-6) **Progressive-anchoring and session boundary.** A reader explicitly operating and disclosing PA-1 MAY resolve against its disclosed, signed in-code registry snapshot only when its trust policy accepts `governance.anchoring: "in-code"`; it MUST NOT describe that result as canonically anchored. A PA-2 or PA-3 reader MUST NOT fall back to in-code constants when the canonical index or a definition is unavailable. Listing-time `verified` establishes discovery eligibility only: at session start the orchestrator MUST resolve and pin the selected rail under DACS-4 §9.4.3 and apply RAV-R1..RAV-R5. A discovery mirror MAY surface availability as an informational hint, but it cannot satisfy that authoritative read.

**Conformance — listing publishers and readers**
A conforming publisher MUST: (LP-1) obtain and verify a CORE §5.1 `finalized` `AnchorReceipt` for each listingVersion, and verify that its native address independently resolves to the expected content hash, before publishing the listing as `active` or referencing it from a listing index; (LP-2) sign the listing with a key referenced by a claim in seller.identity.claims; (LP-3) use monotonic listingVersion values per listingId; (LP-4) publish and retain revocation markers and bindings per RB-1..RB-3; and (LP-6) for a pay-bearing listing, obtain `verified` under LRR-1..LRR-6 at publication time rather than treating its own `acceptedRails` array as proof. A submitted, broadcast-acknowledged, merely accepted, or merely index-visible listing does not satisfy LP-1. A deterministic-BFT binding may establish `included` and `finalized` in the same receipt per CORE §5.1. It SHOULD: (LP-5) probe and maintain an actionable engagement surface for every actively published record.
...
**Self-declared or unregistered payment rail.** *Threat:* a signed listing names a rail in `acceptedRails` and its pipeline, but the rail is absent from the canonical registry, has a different registered phase handler, or is available only through an undisclosed local fallback. *Mitigation:* LRR-1..LRR-6 treat the listing fields only as claims, resolve every accepted rail independently through the authenticated registry, bind each pay phase to the registered handler, and keep unavailable authority `indeterminate`. PA-2/PA-3 readers never fall back silently to in-code constants.
