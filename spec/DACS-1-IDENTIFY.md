...
A reader encountering an unknown scheme MUST: preserve the reference verbatim when forwarding; treat the reference as **unverified** for evaluation purposes; NOT silently accept the reference as satisfying a bundle requirement; log or surface the unknown scheme to the calling agent. A reader MAY decline to engage with a bundle that contains an unknown scheme in a required position. The single named exception is the legacy `web2:domain:<host>` read alias resolved by the §6.3.2 canonical claim-reference resolution rule: a `web2:domain:` reference that resolves to a canonical `domain:` effective reference is **not** treated as unknown; no other unregistered scheme gains this treatment.
...
**Canonical claim-reference resolution and the `web2:domain:` read alias (normative).** `domain:<lowercase-IDNA-host>` is the single canonical DNS-domain claim reference; new producers MUST emit only this form. The legacy `web2:domain:<host>` string (the Demos GCR `web2.domain` source representation, §A.1) is a **permanent read/replay alias**, not a second canonical form. A reader resolves a bundle in three ordered layers:

- **Layer 1 — integrity.** Verify the original signed bytes first: NFC/CF-1 → RFC 8785 JCS → `bundle_hash` → presentation signature (§6.3.2), over the bytes **as received**. Alias resolution MUST NOT run at this layer, and a reader **MUST NOT rewrite or re-hash the historical signed artifact** — the `web2:domain:` bytes stay verbatim in the hashed/signed form.
- **Layer 2 — evaluation.** After the signature verifies, each claim's `ref` is CF-2-canonicalised into an **effective claim reference**. A `web2:domain:<host>` ref resolves to the effective reference `domain:<host'>` iff `<host>` lowercased is a valid canonical domain identifier (the §6.3.1 `domain:<dns>` form); otherwise it resolves to nothing and the claim stays unverified (fail-closed). A `domain:<host>` ref is its own effective reference after CF-2. All requirement matching (§6.3.3), tier derivation (§6.3.2.1), `presentedBy` resolution, reputation keying, and the step-4 identifier match consume the **effective** reference, never the raw alias string.
- **Layer 3 — identity.** CF-3 canonical identity `(Scheme, Identifier)` is computed on the effective reference.

The alias is read-only: a reader accepts a historical `web2:domain:` claim by resolving it, but a producer MUST NOT emit it going forward (§6.3.3 producer rule).
...
4. **Identifier match** — `VerifyResult.identifier` matches the identifier component of the claim's **effective claim reference** canonically (the effective reference defined by the canonical claim-reference resolution rule below; for a claim whose `ref` is already canonical this is just the `ref` identifier). A `web2:domain:a.example` claim whose `verifiedBy` verified `b.example` fails this step — the verification does not transfer across a host mismatch, so the claim is unverified.
...
- **(CR-1) `domain:` claim verified by `demos-gcr-domain` control binding.** For a `domain:` claim whose `verifiedBy` resolves to a `demos-gcr-domain` VerifyResult (DACS-2 §7.3.10), controlled use (serving as the bundle's `presentedBy`, and reputation keying against it) additionally requires the **presentation to verify under the exact Ed25519 account bound in the authenticated GCR record** (`gcrRecord.account`, DACS-2 §7.3.10 GE-6/GE-5) — either **directly** as the presentation signing key (§6.3.2 / §B.7), or via an **authenticated SR-1 linkage / session binding** to that account. A `demos-gcr-domain` `pass` establishes that account A registered the domain; it does **not**, on its own, prove that the *presenter* is A. So a GCR entry proving account A registered a domain MUST NOT make a bundle signed by an unrelated account B a **controlled** domain claim: such a claim remains **valid-but-uncontrolled** — it MAY satisfy a required claim (existence/validity), but it MUST NOT be the `presentedBy` claim and reputation MUST NOT key against it. This is the same "control follows the proof, not the storage" principle applied to the substrate-native binding: the authenticated record is authority for *who registered the host*, not for *who is presenting the bundle*.
  - **SR-1 control hook (substrate-bound).** The "authenticated SR-1 linkage / session binding to `gcrRecord.account`" is a **substrate-supplied predicate** in the same style as the DACS-2 §7.3.10 SB-1/SB-2/SB-3 hooks: its concrete Demos wire format (how a presentation proves an authenticated binding to a Demos account) is **not yet fixed and binds at the substrate profile**. **Security property the hook MUST establish (normative, format-independent):** the linkage MUST demonstrate **possession of the private key for `gcrRecord.account`, bound to *this* presentation** — i.e. a fresh proof of key control that is cryptographically tied to the bundle/presentation being evaluated (for example a challenge or the presentation itself signed by that account's key), such that the proof cannot be replayed against a different presentation. A linkage that does **not** demonstrate presentation-bound key possession MUST NOT satisfy it: in particular a **bearer token**, a **long-lived or ambient session**, or an **authorization issued for a different purpose** (however "authenticated" in some other sense) MUST NOT confer control — an unauthenticated or absent linkage likewise MUST NOT. The wire format is not specified here; a conforming implementation binds it to the concrete SR-1 realisation at its substrate profile, which MUST realise this security property. (DACS-2 §7.3.10 references this hook from the `demos-gcr-domain` method side.)
...
**Semantic-claim deduplication (normative — new in v0.6).** The matching algorithm above keys on the claim's **effective** reference (§6.3.2 canonical claim-reference resolution). Two claims that resolve to the same CF-3 canonical identity `(Scheme, Identifier)` — for one hostname, a `domain:<host>` claim and a legacy `web2:domain:<host>` claim both resolve to `domain:<host>` — represent **one semantic claim**, not two. Readers MUST apply:

1. **Identity equivalence** — claims sharing a canonical effective identity are one party/claim. A bundle carrying both alias forms for a hostname MUST NOT gain a second claim, a higher tier, or any additional requirement satisfaction from the duplication.
2. **Evidence is independent and never transferred** — each claim's `verifiedBy` is resolved on its own terms (including the step-4 identifier match against **its own** effective reference). A verified `domain:<host>` alongside an unverified legacy `web2:domain:<host>` does not make the alias verified, and vice versa; the semantic claim is verified iff at least one equivalent claim is verified-and-fresh.
3. **Conflict fails closed** — if two equivalent claims resolve to contradictory decisions for the same identity (one `pass`, one `fail`), the semantic claim is **unverified** for that identity. This blocks laundering a `fail` behind a `pass` alias. (Producing contradictory aliases requires control of one's own bundle, so this is self-harm, not an external griefing vector.)
4. **Producer rule** — a producer MUST NOT emit both the canonical `domain:<host>` and the legacy `web2:domain:<host>` form for one hostname in a bundle; a bundle carrying both is non-conforming (readers still resolve it per rule 1 for audit/replay compatibility).
