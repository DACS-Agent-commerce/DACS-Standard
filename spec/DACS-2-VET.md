...
- A **verification method registry** — a closed set (`verifiable-credential`, `tlsnotary`, `zktls`, `consensus-backed-proxy`, `oauth-attested`, `evm-rpc`, `domain-tls-control`, `demos-gcr-domain`, `self-signed`), each with input/output shape, trust model, and substrate requirements.
...
The Demos substrate records a DNS-domain proof natively: the node fetches and verifies a signed `/.well-known/demos-cci.txt` file over HTTPS, and a `web2.domain` entry — a write authorized by the domain owner's account under the substrate's write-authorization relation (SB-2; see the **open question** below on the concrete key that signs it) — is recorded in the counterparty's Global Context Record (GCR / CCI). This method verifies a `domain` claim by **deriving its verdict from authenticated GCR evidence** — it does **not** refetch the well-known file, and it does **not** trust any caller-asserted host, account, or transaction. A `pass` MUST be derived from evidence whose consensus inclusion, finality, write-authorization, **and the provenance of the presented `gcrRecord` from the state that `sourceTx` actually wrote** are authenticated against the substrate; **indexer visibility alone is NOT authority**, and neither is a record that is merely internally self-consistent (GE-8) but not authenticated as chain-written state (GE-10). It verifies a host↔account binding **authenticated at write time** for the `domain` scheme — see the **Limitation** below: this is a snapshot at write time, not a current-ownership or revocation check. It is not a fresh per-session domain-control challenge.

```
type AuthenticatedGCRDomainEvidence = {
  substrate: string                    // which Demos network (e.g. "demos-mainnet" / "demos-testnet")
  context: "web2.domain"               // the Demos GCR context this evidence is drawn from
  inclusionProof: InclusionProof       // consensus inclusion + finality of sourceTx (SB-1)
  sourceTx: string                     // the GCR-writing transaction
  txWriter: string                     // ed25519 account that signed sourceTx (SB-2)
  gcrRecord: {                         // the binding AS AUTHENTICATED on-chain
    host: string                       //   the bound DNS host
    account: string                    //   the bound ed25519 account
    proofUrl: string                   //   the well-known proof URL the record carries
  }
  recordRef: AttestationRef            // anchored snapshot; its contentHash is over gcrRecord
  recordedAt: number                   // consensus inclusion / recorded time (SB-3)
  availability: "resolved" | "not-found" | "transport-error" | "record-unreadable"
}

type DemosGCRDomainMethodInput = {
  recipe: Recipe
  identifier: string                   // the canonical A-label host (DACS-1 §6.3.1 domain form)
  claimAccount: string                 // the DACS-1 claim's asserted ed25519 account (GE-6 right-hand side)
  evidence: AuthenticatedGCRDomainEvidence  // the authenticated web2.domain GCR evidence
}
```

The bound account is `evidence.gcrRecord.account` and the bound host is `evidence.gcrRecord.host`; there is **no** separate caller-supplied `boundAccount`/`boundHost`/`sourceTx` trust point — the earlier caller-asserted fields are gone.

**Procedure (authenticated binding checks, GE-1..GE-10).** A `pass` MUST be derived from the authenticated evidence, and **all** of the following MUST hold; any one violated MUST NOT yield `pass`:

- **(GE-1)** `evidence.context` equals the recipe's `gcrContext`.
- **(GE-2)** `evidence.substrate` equals the recipe's pinned substrate.
- **(GE-3)** `evidence.inclusionProof` covers `evidence.sourceTx` (SB-1) — the proof is about the transaction being relied on.
- **(GE-4)** `evidence.inclusionProof` meets the substrate finality criterion (SB-1) — an unfinalized write is not yet authority.
- **(GE-5)** the write of the `web2.domain` entry MUST be **authorized by `evidence.gcrRecord.account` under the substrate's write-authorization relation (SB-2)** — a third party cannot write another account's binding. GE-5 is defined in terms of that relation, NOT raw key equality: where the substrate realises the write via a node or system key acting on the bound account's authenticated instruction, SB-2's relation is what GE-5 checks (`evidence.txWriter` may legitimately be a node/system key); where the substrate requires the bound account itself to sign `sourceTx`, the relation reduces to `evidence.txWriter == evidence.gcrRecord.account`. See the **open question** below.
- **(GE-6)** `evidence.gcrRecord.account == claimAccount` (the DACS-1 claim's asserted ed25519 account, carried in `DemosGCRDomainMethodInput`).
- **(GE-7)** `evidence.gcrRecord.host == identifier`.
- **(GE-8)** `evidence.recordRef.contentHash == hash(evidence.gcrRecord)` — the anchored snapshot is over exactly this record. This is **self-consistency of the presented record only** (see GE-10 for provenance).
- **(GE-9)** `evidence.gcrRecord.proofUrl == derive(identifier, recipe.proofPathTemplate)`. The authenticated record **carries** the proof URL; the local derivation is the **check**, never the source of truth (LIST-C).
- **(GE-10)** the `gcrRecord` MUST be the `web2.domain` state that `evidence.sourceTx` actually wrote, as established by `evidence.inclusionProof` (SB-1). This is a **DISTINCT property from GE-8**: GE-8 proves the presented record is self-consistent with its own `contentHash`; GE-10 proves that record is the *provenance-authenticated* state recovered from the transaction on chain. A record that is merely self-consistent — correct internal hash, `host` = the claimed identifier, `account` = the claim account, `proofUrl` = the correct derivation — but is **not authenticated as the state `sourceTx` wrote** MUST NOT yield `pass`. GE-3/GE-4 authenticate that *a* transaction exists and is final; GE-10 binds *this record* to *that transaction's written state*.
- **(SB-3)** `evidence.recordedAt` is present and consistent with the inclusion proof.

Indexer visibility alone is NOT authority: a record an indexer merely returns, without an inclusion proof meeting GE-3/GE-4, is unauthenticated and MUST NOT yield `pass`. Returning, per the §7.5.1 decision values (this method's mapping only; the global §7.5.1 taxonomy and every other method's mapping are unchanged):

- **`pass`** (§7.5.1) — all GE-1..GE-10 checks hold.
- **`fail`** (§7.5.1) — the authenticated evidence **conclusively contradicts** the claim: wrong host (GE-7), wrong account (GE-6), a write not authorized by the bound account under SB-2 (GE-5), wrong context/substrate (GE-1/GE-2), tampered/forged evidence (GE-8 contentHash mismatch, GE-9 proofUrl mismatch), or the transaction authenticates a **different** `web2.domain` record for that host than the one presented (GE-10 conclusive contradiction). The authority ran to completion and its authenticated answer contradicts the claim.
- **`indeterminate`** (§7.5.1) — authority-side unavailability or **unauthenticatable** evidence: `not-found`, `transport-error`, `record-unreadable`, read timeout, a missing or unverifiable inclusion proof (GE-3), an unfinalized write (GE-4), a record whose **provenance from `sourceTx` cannot be established at all** (GE-10 linkage unauthenticatable — the presented record is neither confirmed nor contradicted as the written state), or otherwise incomplete evidence. LIST-A: transport/read unavailability is an inconclusive **answer**, not a verifier defect — an unreachable authority yields `indeterminate`, not `error`. Per DEMOS-MAPPING §A.2 (authoritative absence), a Demos `not found` carries the SR-2 disposition `indeterminate`, not `absent` — there is no authenticated non-membership proof — so it is `indeterminate`, **never `fail`**.
- **`error`** (§7.5.1) — narrowed to a **verifier-internal** fault only: the verifier cannot run the check at all. It is never used for an unreachable or unreadable authority (those are `indeterminate`).

**Substrate-binding hooks (SB-1/SB-2/SB-3).** Three substrate-supplied predicates carry the authority of this method, and their concrete Demos wire formats are **not yet fixed** — they bind at the substrate profile and are modelled synthetically by the conformance vectors:
- **SB-1** — the structure of `inclusionProof`, the Demos finality criterion (what "covers `sourceTx`" and "finalized" mean on-chain), **and how the `web2.domain` state written by `sourceTx` is recovered from that authenticated transaction** (the provenance GE-10 relies on — how, given an authenticated final transaction, an implementation obtains the record that transaction wrote and compares it to the presented `gcrRecord`). The `InclusionProof` type referenced in `AuthenticatedGCRDomainEvidence` is **not defined in this document**: its concrete shape, and the concrete state-recovery mechanism, are bound by SB-1 at the substrate profile. A verifier MUST treat an `inclusionProof` it cannot authenticate against the substrate (missing, malformed, not covering `sourceTx`/not finalized), or a record whose provenance from `sourceTx` it cannot recover, as unverifiable evidence (`indeterminate`, GE-3/GE-4/GE-10), never as a pass.
- **SB-2** — recovery of the transaction writer from `sourceTx`, and the substrate's write-authorization relation for a `web2.domain` entry (whether a write is authorized by the bound account — see the open question below on the concrete realising key).
- **SB-3** — the wire representation, epoch, and precision of the consensus record time `recordedAt`.

A conforming implementation binds SB-1/SB-2/SB-3 to the concrete Demos formats at its substrate profile; this document names the hooks and requires that GE-3/GE-4/GE-5/GE-10/SB-3 be evaluated against authenticated substrate data, not presenter assertion. A `demos-gcr-domain` `pass` establishes *who registered the host*; whether the *presenter* controls that account — the gate for using the claim as `presentedBy` or a reputation key — is the separate **DACS-1 §6.3.2 CR-1** control binding, which relies on its own substrate-bound **SR-1 control hook** (declared in DACS-1 §6.3.2).

> **OPEN QUESTION (unsettled — referred to the DACS-2 steward).** The concrete Demos **write-authorization realisation** for a `web2.domain` entry is not yet settled: whether the entry is written by the domain owner's **account key** (so `evidence.txWriter` is that account and SB-2 reduces to `txWriter == gcrRecord.account`), or by a Demos **node / system key** acting on the owner's authenticated instruction (so `evidence.txWriter` is a node key and SB-2 is the substrate's node-authorization relation, not raw equality). GE-5 is written against the SB-2 **relation** precisely so it holds either way; SB-2 binds the concrete realisation at the substrate profile once the steward settles it. Until then, an implementation MUST NOT assume raw `txWriter == gcrRecord.account` equality unless its substrate profile establishes that the account key signs the write. *(Also open, and out of scope here: selecting among multiple concurrent or historical `web2.domain` records for one host — see §7.4 governance.)*

**Metadata.** On `pass`, the result's `data` carries the binding's public context for downstream inspection: `proofUrl` (= `evidence.gcrRecord.proofUrl`, having passed GE-9 — the authenticated record's proof URL, **verified equal to** `derive(identifier, recipe.proofPathTemplate)`, not merely derived), `boundAccount` (= `evidence.gcrRecord.account`, the authenticated bound account), `gcrRecordRef` (= `evidence.recordRef`), `sourceTx`, and `recordedAt`. These are **already-public authority fields** under the §7.5 public-anchor minimisation rule (a domain name, an on-chain account, a well-known URL, a public transaction hash, and a consensus record time leak nothing beyond what is already public), so they are minimisation-clean and MAY be carried in cleartext `data`. *Caveat:* a deployment that exposes private GCR slots as `web2.domain` MUST NOT copy those private values into a publicly-anchored `data` — the same reduction-to-predicate rule that governs any private extracted field applies.
...
**Freshness (GF-1/GF-2/GF-3).** The effective window is derived from the authenticated evidence time, never from a presenter-supplied value:

- **(GF-1)** `verifiedAt := evidence.recordedAt`. It is **never presenter-supplied**. A presenter-supplied `verifiedAt` that disagrees with `recordedAt` makes the claim **unverified** — the **anti-reissue gate**: the same historical GCR entry MUST NOT be reissuable with a fresh window by re-presenting it under a newer asserted time.
- **(GF-2)** `validUntil := verifiedAt + the recipe window` (`defaultMaxAgeSec`, §7.4.1), tightened only **downward** by `ClaimRequirement.maxAge` (§6.3.3). There is no expiry beyond the recipe window **measured from evidence time**.
- **(GF-3) recordedAt sanity.** Two conditions, both fail-closed on violation: **(i)** `evidence.recordedAt` MUST NOT exceed `now + recordedAtSkewToleranceSec` — a `recordedAt` further in the future than the tolerance makes the claim **unverified**; and **(ii)** the derived window MUST be **positive** (`validUntil > verifiedAt`) — a zero-length or negative window makes the claim **unverified**. A window whose start (`verifiedAt := recordedAt`) is at most `recordedAtSkewToleranceSec` ahead of the verifier's `now` **IS acceptable** — permitting exactly this is the whole point of the tolerance, so (i) and (ii) are not in tension. The freshness gate treats such a within-tolerance-future window as already open: its lower bound is relaxed by `recordedAtSkewToleranceSec` (`verifiedAt − recordedAtSkewToleranceSec ≤ now ≤ validUntil`). There is deliberately **no** separate "window begins after `now`" rejection: a future `verifiedAt` is bounded solely by condition (i), and anything within tolerance passes. The tolerance is a recipe/profile parameter (`recordedAtSkewToleranceSec`), not a hard-coded constant; it exists only to absorb ordinary verifier-vs-consensus clock skew, not to admit materially future-dated evidence — and is hard-bounded by the **Tolerance bound** below so a profile cannot enlarge it enough to nullify GF-3.
- A separate query/fetch time (`queriedAt`) is optional metadata only: it MUST NOT extend the window and MUST NOT substitute for `recordedAt`.

**Tolerance bound (GF-3, normative).** A recipe MUST set `recordedAtSkewToleranceSec ≤ defaultMaxAgeSec / 100` — at most **1%** of the recipe window. A recipe whose `recordedAtSkewToleranceSec` exceeds this bound is **non-conforming**: it MUST be rejected at recipe authoring/resolution (§7.4.3) and MUST NOT be anchored; a consumer that encounters such a recipe MUST treat it as invalid rather than honour the oversized tolerance. This is a hard constraint a conformance checker evaluates directly on the two recipe fields, not advisory guidance. The **1%** ratio is chosen because it is small enough that the tolerance can never meaningfully extend the effective freshness window — a within-tolerance claim gains at most 1% grace on expiry, and at most 1% of the window is admissible as future-dated, so an oversized tolerance cannot nullify GF-3 — yet large enough to absorb genuine verifier-vs-consensus clock skew for any realistic window (e.g. ~100 minutes of skew tolerance on a 7-day window).

**Clock-trust assumption (GF-3).** GF-3 is the one point where this method trusts the **verifier's local clock**, and only for the narrow future-dating sanity bound above, within `recordedAtSkewToleranceSec`. `recordedAt` itself is the substrate's consensus-attested record time and is authoritative for the window (GF-1/GF-2) — consistent with the standard's general discipline that the substrate anchor time, not a wall clock, decides windowing (cf. the sealed-envelope SE-2 chain-timestamp rule, §8.4.3, and the DACS-5 §10.5 windowing-basis discipline). GF-3 does not make the local clock a windowing basis; it only rejects evidence that is implausibly ahead of it.

**Persistence (no session nonce).** The GCR entry is a **write-time-authenticated** host↔account binding, not a fresh per-session challenge: the result carries **no nonce or challenge-binding field**, and its acceptance is governed by the existing DACS-1 §6.3.2 effective-window freshness gate like any other `verifiedBy`. A fresh per-session domain-control check (nonce-bound) is a distinct capability tracked upstream as DEM-767 and MUST NOT be inferred from this record. **Trust model:** DNS + Web PKI **at the node's write time**; Demos consensus for the recorded result and its provenance; ed25519 for the host↔account binding. **Substrate:** SR-2.

**Limitation (no revocation or current-ownership check).** This method authenticates a binding **as of `recordedAt` (write time)** — it does **not** check revocation, supersession, or *current* ownership. A genuine, correctly-authenticated record therefore remains presentable — and yields `pass` — for the whole recipe window **even after the domain has changed hands, been transferred, or lapsed in DNS**. The recipe window (`defaultMaxAgeSec`, GF-2) is the **only** bound on that exposure: nothing in this method observes a later state change. This is a deliberate consequence of consuming a persistent record rather than performing a fresh control proof. Deployments that need **current-ownership** assurance MUST NOT rely on this method's window alone: they SHOULD tighten `defaultMaxAgeSec` to the shortest window their risk tolerates, and/or require the fresh-control method **`domain-tls-control`** (§7.3.8) — which re-proves control per session — instead of, or in addition to, `demos-gcr-domain`.
...
| domain | domain-tls-control (default) | ACME-style; `demos-gcr-domain` is available as an explicitly-selectable alternative method (Demos GCR `web2.domain` binding, §7.3.10) |
...
**`domain` scheme methods (v0.4).** The `domain` scheme now has two active, distinct verification methods that a recipe MAY select explicitly: `domain-tls-control` (fresh per-session ACME-style control proof, §7.3.8) and `demos-gcr-domain` (persistent Demos-GCR `web2.domain` ownership-and-key evidence, §7.3.10). **`domain-tls-control` remains the scheme's default recipe.** This document registers the `demos-gcr-domain` method and defines its normative content; it does **not** change the `domain` scheme's default recipe and does **not** designate any recipe supersession — the two methods coexist as independently-selectable proof families. Any future replacement of the scheme's default recipe is a separate, explicit steward ruling, not implied here. This paragraph defines the method's normative content only; it is **not** itself the signed, anchored Recipe artifact. That artifact — including its `parserRules` and `signature` — is resolved from the steward-controlled `dacs2:registry:v0.1` index and is signed and published by the registry steward (§7.4.4); this document does not write or publish the registry entry.
