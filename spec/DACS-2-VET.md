...
4. if the matched `ClaimRequirement` pins a specific `recipeVersion` (§6.3.3), MUST use that version within the selected family, otherwise MUST use that family's latest version at session start, pinned into the session.
...
2. checking integrity against AttestationRef.contentHash by the same procedure that produced it at anchor time (contentHash is the sha256 of the anchored content's canonical form, per the Content-hash definition):
   ...
   - for a `VetRequirementAuthorization` or
     `ProvenancedCompositeVerificationRecord` — type-dispatching first, then
     hashing the complete signed artifact including `signature`, as explicitly
     required by §§7.7.3–7.7.4; the separate signature-preimage hash does not
     satisfy reference integrity;
...
type CompositeVerificationRecord = {
  ...
  requirementHash: string                     // sha256 of the exact BundleRequirement evaluated; it is listing-owned only for the publisher→counterparty invocation
...
**Anchored-content exception.** For this new type only, the
`AttestationRef.contentHash` and `AnchorReceipt.contentHash` are instead
`sha256(canonical_JCS(complete signed authorization))`, including `signature`.
The signature-preimage `authorization_hash` and anchored-content hash are
distinct values and MUST both be recomputed. A reference or receipt carrying
the former in place of the latter is invalid. This is the explicit CORE §B.2
type-specific exception; it changes no legacy artifact.
...
- (VPA-10) **Anchor gate.** Before verification starts, the handler MUST resolve
  ...
  from the logical form. The receipt MUST also bind the full signed artifact's
  recomputed content hash, transaction, writer, and nonce where the binding
...
**Anchored-content exception.** For this new type only,
`AttestationRef.contentHash` and `AnchorReceipt.contentHash` are
`sha256(canonical_JCS(complete signed record))`, including `signature`. They
MUST NOT carry the signature-preimage `provenanced_composite_hash`. This is the
same explicit CORE §B.2 type-dispatched exception as §7.7.3 and changes no
legacy artifact.
...
- (PVC-6) WN-1..WN-6 apply unchanged. VPC-3 and VPC-5 apply to this record's
  ...
  the full signed record's recomputed content hash, transaction, writer, and
...
| Composite record reader | §7.7.1 aggregation; signature validation; structurally distinguish the legacy and provenanced types; for provenanced records resolve and validate the complete authorization chain |
