...
10. **Claimed replacement** — `terms.priorPaymentDispositionRef` MUST be absent unless this is a fresh-job replacement under a `pay-alternative` Listing. When present, the commitment handler MUST complete APR-6 disposition resolution, signature/writer/finality verification, exact prior Agreement/selection/index binding, and closed-state proof checks before accepting the new Agreement. An unavailable otherwise-consistent disposition is `indeterminate`; an open, malformed, mismatched, or disproven disposition rejects and permits zero replacement-rail authorization calls.
