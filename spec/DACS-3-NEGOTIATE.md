...
3. **Rail** — `terms.rail` MUST be present if and only if the listing pipeline contains a `pay-*` phase (PIPE-1, §9.5). When present, exactly one member of `listing.acceptedRails` MUST have CORE §B.2 RFC 8785 canonical bytes equal to the complete `terms.rail`; selection by `railId`, first matching entry, partial-object comparison, or parameter merging is invalid. The equality includes `railVersion`, `parameters`, and unknown preserved members. DACS-1 LRR-1 separately rejects duplicate canonical listing refs, while permitting distinct refs that share a handler-dispatch `railId`. A selected `x402:protocol` reference MUST have the complete DACS-4 §9.3 negotiated shape and the artifact MUST be a `PayeeBoundAgreementDocument` produced by `commit-payee-bound-agreement`; a legacy `AgreementDocument`, a missing payout binding, or an unsigned runtime payee address MUST be rejected before Settle. For a zero-pay (intake-only / settled-out-of-band) pipeline — one with no `pay-*` phase — `terms.rail` MUST be absent.
