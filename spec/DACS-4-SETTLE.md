...
**Artifact gate and legacy behaviour.** Before interpreting agreement terms, a payer MUST select the DACS-3 artifact schema from its required version discriminator (§8.5). A payer that does not implement `PayeeBoundAgreementDocument` or `SealedSelectionAgreementDocument` MUST reject the unsupported artifact before invoking any pay handler; it MUST NOT discard `payeeBoundAgreementVersion`, `sealedSelectionAgreementVersion`, `terms.payoutBindings`, or `selectionReceiptRef` and retry it as an `AgreementDocument`. A payer implementing the selection-bound type MUST first resolve and reproduce its SAC-8 receipt. An older payer expects a known required discriminator, so the new artifact fails its schema gate and no payment is submitted.
...
**Payee-destination binding (PB-1..PB-3).** The rules below apply when `agreement` is a `PayeeBoundAgreementDocument` or `SealedSelectionAgreementDocument`; the latter also passes SAC-8 before PB evaluation. `payingKey` already binds the payer side to the bundle (`MUST appear in payer's bundle.claims`); PB restores the missing symmetry on the destination.
...
Where no tier is satisfiable for a payee-bound phase, the payer MUST refuse to pay. A missing `payoutBindings` entry is an invalid `PayeeBoundAgreementDocument` or `SealedSelectionAgreementDocument`, not a legacy agreement and not permission to infer a destination.
...
- (APR-5) **Agreement, execution, and evidence binding.** A
  `PayeeBoundAgreementDocument` or `SealedSelectionAgreementDocument` MUST cover the projected concrete payment with
...
**Agreement artifacts across minor versions.** A legacy DACS-4 v0.2 payer continues to accept `AgreementDocument` and use the runtime `payeeAddress` under its existing semantics. It rejects `PayeeBoundAgreementDocument` at the required `agreementVersion` schema gate and therefore cannot ignore PB and pay anyway. A DACS-4 v0.3 payer recognises both earlier artifacts and applies PB only to the payee-bound type. DACS-4 v0.8 adds `SealedSelectionAgreementDocument`, applies PB and SAC-8 to it, and leaves both earlier types unchanged. Older payers reject its distinct discriminator before payment. This is additive new-type refusal under CORE §11.1.2, not a semantic change to an earlier artifact.
...
**Payee-destination substitution.** *Threat:* a tampered listing, compromised negotiation channel, or malicious orchestrator substitutes `payeeAddress` so funds go to an attacker while every identity check passes. *Mitigation:* for either payout-bearing agreement type, PB-1 carries the destination inside the co-signed artifact (a substituted address breaks the agreement hash; an honest payee never co-signs an attacker's address), and PB-2 binds it to the vetted identity by the strongest applicable tier — intrinsic, control-proven `cci-xm:` linkage (§6.3.2 step (6)), or the payee's own agreement signature; applicable-but-unresolvable pauses rather than paying (§9.5.1). The distinct artifact and phase make older readers reject before settlement rather than ignore PB; the sealed-selection type additionally gates on SAC-8. *Residual:* a legacy `AgreementDocument` has no PB guarantee; a payee asserting a tier-3 address it does not control bears the payee-side risk, visible via the recorded binding tier.
