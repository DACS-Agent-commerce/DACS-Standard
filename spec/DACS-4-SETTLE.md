...
Before step 1 or any rail resolution, hash derivation, provider-metadata construction, AP2-7 binding reservation, or provider call, the handler MUST pass CORE §11.1.2 authenticated corrective-profile admission, validate `jobId` under JID-1, and require `phaseIndex` to be a non-negative integer. Caller-supplied profile fields, unauthenticated peer claims, successful signature verification, or a major-only version discriminator MUST NOT substitute for authenticated exact-pin and complete-module-tuple equality.
