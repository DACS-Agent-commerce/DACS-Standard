...
- (RAV-R2) For a new production session, an orchestrator MUST NOT select a rail with availability `mocked`, `disabled`, or `failed`. A session that already pinned a rail MAY continue when a later registry revision marks that rail `disabled`; the orchestrator MUST NOT reinterpret that permission as authority to create a replacement or additional session. `failed` settlement attempts remain subject to RAV-R4.
