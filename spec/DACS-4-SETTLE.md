...
The optional Atomic Work candidate adds the `pay-dem` payment slot and structurally distinct Work-operation settlement evidence without weakening the current v0.8 payment, delivery, finality, or compatibility contracts.
...
the PDE-7 legacy arm. A capability-gated CORE
...
new DACS-4 v0.7 producers MUST NOT emit it for a delivery phase. Atomic
`pay-dem` and Atomic `deliver-storage-program` use the additive §9.7.3
`AtomicSettlementEvidenceV1` artifact instead. All three evidence kinds are
anchored on the substrate and referenced by DACS-5.
...
- (PIPE-6) Except for a `dacs-purchase-v1` Work that satisfies every DACS-3
  AWP-6 through AWP-11 co-finality condition, before executing any effective concrete pay-* phase
