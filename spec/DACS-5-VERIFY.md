...
**Stage:** Verify (5th of 5). **Status:** Draft — **DACS-5 v0.6** on the common DACS v0.1 baseline. v0.6 consumes the structurally distinct DACS-3 `SealedSelectionAgreementDocument` and requires SAC-8 receipt reproduction before its winner or price can enter audit or settlement-verified reputation; v0.5 participates in the declared CORE §11.1.2 pre-v1 corrective boundary, applies JID-1..JID-4 to session and bundle resolution, makes the §10.4.2 bundle-address preimage byte-exact, consumes phase-bound `DeliveryEvidence` with exact PDE-8 invocation mapping and a narrow single-delivery legacy read arm, and makes APR-7 effective-pipeline recomputation mandatory for `pay-alternative` Listings before phase-summary or evidence admission; it is not live-compatible with a pre-JID-1 profile. v0.4 added the non-terminal `audit-pending` gate, required every successful bundle dependency plus the completed bundle itself to be finalized and independently resolvable, added the `EvidenceBoundFaultAttestationBundle` type with SEB-1..SEB-6 exact settlement-evidence binding, and added structurally distinct settlement-verified reputation derivation types while preserving the released v0.3 `ReputationDerivation` and `ReplayableReputationDerivation` version-1 semantics. v0.3 added `PayeeBoundAgreementDocument` consumption alongside the legacy agreement artifact, the signed `BundleBinding` artifact with BB-1..BB-8 logical→native bundle resolution §10.4.2, and the `FaultAttestationBundle` artifact — absolute hashed `faultedParty` attribution as a distinct type under its own `dacs-fault-bundle:v1:` domain §10.4.1. **Depends on:** SR-1 for cross-substrate primary-claim keying, SR-2 for bundle anchoring; composes with the ERC-8004 reputation registry as an OPTIONAL publication surface. **Used by:** all subsequent DACS-1 reputation lookups, external auditors and regulators.
...
`evidenceBoundFaultBundleVersion`, with value `"1"`, and that selector MUST
...
dispatcher. Implementations SHOULD expose fixed direct-full, FAB-pointer, and
EBFAB-pointer operations. A general shared-address dispatcher remains
...
- **re-run RSV admission (settlement-verified type only)** — resolve and independently verify every presented payment or delivery evidence artifact under RSV-1/RSV-2 from immutable, hash-bound or finalized-state-bound authority; require `verified` before comparing metrics. `rejected` or `indeterminate` makes the receipt unverifiable and MUST NOT produce an alternative metric set;
...
- (RSV-1) `verify_presented_settlement_evidence` MUST resolve every `AttestationRef` in `authoritative.settlementEvidence`, classify the artifact by its exclusive DACS-4 §9.7 discriminator before type-specific action, verify its content hash and type-specific signature domain, and return exactly `verified`, `rejected`, or `indeterminate`. Payment members are `SettlementEvidence`; current delivery members are `DeliveryEvidence`; the PDE-7 legacy delivery arm remains narrow and read-only. Whether the presented multiset is complete is the separate §10.4.3 production rule, not this guard.
- (RSV-2) Each resolved artifact MUST pass the applicable DACS-4 consumer rules against authority independent of the evidence under test: the authenticated Agreement and session, executed phase index, phase-specific logical addresses, SB-1 through SB-3, and either the payment rail/asset/network, resolved transaction parties/destination/amount and finality checks or the delivery PDE/DPA artifact-resolution chain. When a payment rail requires SB-3 binding, an absent or unavailable binding is `indeterminate` and the job is excluded under RSV-3; unbound transfer evidence cannot make it reputation-eligible. The deriver MUST NOT infer expected economics or delivery validity from the evidence record itself, from the outer bundle's signatures, or from `fetch_and_verify_agreement` alone.
...
| Bundle consumer | Resolve native addresses per BB-4..BB-8 (verify bindings and role authorization, prune to the co-signed party map where available, apply the authorized-candidate multiplicity rule, fail closed to `indeterminate`; one-sided classification only after a resolved binding plus policy-qualified authoritative absence); establish full-bundle versus explicit pointer operation and family from protocol-owned context or unique registered-domain verification before requiring exactly one matching supported selector; never treat BundleBinding or a caller label as family authority; reject a copy whose `faultedParty` contradicts its (outcome, anchoredByRole); run SEB-1..SEB-6 on EBFAB before pair selection and apply PDE-8 whenever current `DeliveryEvidence` is present; recompute canonical hashes, verify domain-separated signatures, and dereference and validate every contained `AttestationRef`; reconcile by EBFAB > FAB > legacy only after validity and non-divergence |
...
**Cross-protocol and cross-type signature confusion.** *Threat:* a bundle signature is replayed as another DACS artifact or an EBFAB discriminator is stripped/renamed so its signatures are tried against an older bundle type. *Mitigation:* §B.7 assigns `dacs-bundle:v1:`, `dacs-fault-bundle:v1:`, and `dacs-evidence-bound-fault-bundle:v1:` to the three bundle types. A consumer first establishes family through its fixed operation/address contract or a unique successful registered-domain verification, then requires exactly one matching supported selector. It never uses the selector or a caller label to choose which domain to trust. A signature produced under any other artifact or bundle type cannot validate even when the remaining fields or hash bytes align.
