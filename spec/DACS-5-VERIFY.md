...
- **re-run RSV admission (settlement-verified type only)** — resolve and independently verify every presented `SettlementEvidenceRecord` under RSV-1/RSV-2 from immutable, hash-bound or finalized-state-bound authority; a `FinalityBoundSettlementEvidence` additionally re-runs DACS-4 FV-1..FV-10 from its exact signed rail profile and fresh/authenticated proof context. Require `verified` before comparing metrics. `rejected`, `indeterminate`, or `error` makes the receipt unverifiable and MUST NOT produce an alternative metric set;
...
- (RSV-1) `verify_presented_settlement_evidence` MUST resolve every `AttestationRef` in `authoritative.settlementEvidence`, classify the exclusive DACS-4 evidence discriminator, verify its content hash and matching signature domain, and return exactly `verified`, `rejected`, `indeterminate`, or `error`. Whether the presented multiset is complete is the separate §10.4.3 production rule, not this guard.
- (RSV-2) Each resolved artifact MUST pass the applicable DACS-4 consumer rules against authority independent of the evidence under test: the authenticated Agreement and session, executed phase index, pinned rail/asset/network, resolved transaction parties/destination/amount, and SB-1 through SB-3. A `FinalityBoundSettlementEvidence` MUST additionally pass the exact DACS-4 FV-1..FV-10 algorithm using its authenticated `railDefinitionRef` and proof context. The deriver MUST NOT infer expected economics or finality from the evidence's `SettlementFinalityRecord`, the outer bundle's signatures, or `fetch_and_verify_agreement` alone.
- (RSV-3) If any presented artifact is `rejected`, `indeterminate`, or `error`, the deriver MUST exclude the entire jobId from every metric for that derivation. It MUST NOT convert the semantic contradiction, malformed proof, or unavailable authority into a new outcome or fault attribution; exclusion removes the job from numerator, denominators, volume, ratings, `bundleCount`, and `bundleRefs` alike.
...
For `ReplayableSettlementVerifiedReputationDerivation`, RSV is re-executed from the hash-bound `settlementEvidence[]`, Agreement/session/phase/rail authority, and finalized transaction evidence before metric comparison. `rejected`, `indeterminate`, or `error` at replay makes the receipt unverifiable; a replayer MUST NOT silently recompute a different metric set. For finality-bound evidence this includes a fresh FV execution: the producer's earlier pass is not a trusted disposition to copy into `resolutionContext`. An authority input that is not immutable or bound to the finalized state used by the verification cannot produce `verified`.
...
**Finality-bound settlement evidence.** A DACS-5 v0.5 reader does not know the
`finalityBoundEvidenceVersion` discriminator and rejects that evidence before
SEB/RSV action. A v0.6 reader verifies its distinct signature domain and calls
the DACS-4 FV verifier. Historical `SettlementEvidence` remains auditable but
is not relabelled as carrying the new independently verified finality claim.
