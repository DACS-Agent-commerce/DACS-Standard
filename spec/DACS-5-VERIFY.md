...
- (SEB-3) Resolve every top-level reference and authenticate its binding to this bundle's `jobId`, its DACS-4 §9.7 phase kind, and its phase index recovered from the SB-1 evidence anchor. Each resolved member MUST map to exactly one key in `P`; a non-evidence phase, another session, or an unknown/mismatched phase key is rejected.
...
| --- | --- | --- |
| EBFAB / EBFAB | absolute-fault + shared `phaseSummary`; both copies pass SEB | scored party's own copy; either for a non-scoring auditor |
