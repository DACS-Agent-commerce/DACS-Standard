...
**Authenticated-window current profile (AWT-1..AWT-8 plus SPA-1..SPA-8; normative).** `AuthenticatedWindowReputationDerivation` is the only current reputation-bearing DACS-5 profile. It is settlement-verified, job-bound, replayable, participation-safe for one-sided timeout blame, and exact-rate-phase-bound from inception. The five older derivation shapes remain frozen for compatibility and are historical/partial signals. None can satisfy a request for current-profile reputation.

- (AWT-1) A current-profile object MUST carry exactly `authenticatedWindowDerivationVersion: "1"` and `windowingBasis: "verified-business-outcome-occurrence"`. It MUST NOT carry another derivation discriminator. Unknown, missing, or multiply-present discriminators are rejected before metrics are used.
...
A deriver publishing a released-semantics replayable receipt emits `replayableDerivationVersion: "1"` in place of `derivationVersion`. A deriver making the stronger requested-session binding claim while retaining released metric semantics emits `jobBoundReplayableDerivationVersion: "1"`. A settlement-verified replayable receipt emits `replayableSettlementVerifiedDerivationVersion: "1"` and no other derivation discriminator. A current-profile receipt emits `authenticatedWindowDerivationVersion: "1"`. Every job-bound form includes `resolvedJobId` in every context entry. The current form additionally includes `anchorReceipt`, `anchorReceiptHistory`, `outcomeTimePolicy`, `outcomeTimeEvidence`, `outcomeTimeEvidenceHistory`, and conditional SPA-8 `participationEvidence`. All four set `resolutionContext := [entry(b) for b in reconciled]` in the same canonical ascending-`contentHash` order as `bundleRefs`.
...
- **authenticated-window type only** — perform the shared AWT-2 through AWT-7 gate over every current entry and require its verified business-outcome occurrence to remain inside the receipt's inclusive window before metric comparison.
...
`derive_authenticated_window` is the current-profile path. It applies the same validation, two-sided reconciliation, authoritative-copy selection, RSV admission, outcome formulas, and canonical ordering as `derive_settlement_verified`, then adds SPA participation/rating admission. It changes the order and source of window admission as follows:
...
A fifth guard applies only to `derive_authenticated_window`: if that one-copy job attributes an ST-9 timeout abort to an absent non-signer, it enters the current `reconciled` set only after SPA-1..SPA-6 verify. Authoritative absence, a signed admission, or deadline passage alone cannot satisfy the exact-outcome limb. Any rejected or indeterminate participation/outcome dependency excludes the job from all current metrics without manufacturing a different fault. The five pre-current algorithms execute none of this guard and retain their frozen historical behavior.
...
**Versioning.** DACS-5 v0.3 and both original version-1 derivation types were released in the DACS v0.4 profile at `4bb9e48a1095ab32c06c25b7c0b52018d3ce4091`. RSV changed which bundles enter metrics and therefore shipped additively in DACS-5 v0.4 only through `settlementVerifiedDerivationVersion: "1"` and `replayableSettlementVerifiedDerivationVersion: "1"`; the job-bound replay type was likewise additive. AWT changes window admission and replay evidence, so DACS-5 v0.6 added `authenticatedWindowDerivationVersion: "1"` rather than changing any of those five shapes. SPA is integrated only into that unreleased current profile in v0.7 and introduces new `SessionParticipationAdmission`/`TimeoutMarker` types; it does not alter any of the five historical algorithms or released signed record meanings. Existing discriminators retain their released meaning. Any future action-bearing change to a released derivation type requires another structurally distinguishable type or a major compatibility path under CORE §11.1.2.
...
Only the remaining records’ values, whose target matches the scored party, are aggregated; the metric is null when no qualifying ratings exist. These are the frozen pre-current rules. `derive_authenticated_window` additionally applies SPA-7's shared closed artifact/global-roster verifier, exact successful rate-phase relation, exact unsigned-JCS reference hash, fully signed completed-bundle gate, and AWT-verified completed-outcome occurrence before a rating becomes current-window countable.
...
| Reputation deriver | For current decisions emit only `AuthenticatedWindowReputationDerivation`: apply RSV-1..RSV-4, require job-bound replay context, and enforce AWT-1..AWT-8 plus SPA-1..SPA-8 through the shared exact-participation/outcome gate; require source/challenge/admission evidence and independently trusted exact nonresponse/abort evidence before timeout blame; preserve finalized anchor receipts as provenance rather than occurrence time; preserve every older discriminator as historical/partial without reinterpretation; partition by primary claim; treat failed-substrate per the denominator rule; return null for zero-denominator scalar metrics; set `bundleRefs` to exactly the applicable algorithm's `reconciled` set in canonical ascending-`contentHash` order, and emit a derivation reproducible byte-for-byte from `bundleRefs` plus `resolutionContext` per §10.5.3 |
| Rate phase handler | One RatingRecord per direction; reject out-of-range `value` (non-integer or ∉[1,5]) / over-length `freeText` before anchoring (RT-1); anchor each; include in bundle; current-profile consumption additionally applies SPA-7 |
...
**Time-bound reputation windows.** *Threat:* an old, no-longer-representative outcome is presented as current; delayed bundle publication refreshes it; or a producer cherry-picks occurrence or replacement history across a boundary. *Mitigation:* current-profile derivation enforces AWT-1..AWT-8 after reconciliation and RSV. One shared gate verifies CORE-valid anchor provenance and independently policy-proven business occurrence for the exact session outcome. It uses inclusive boundaries and makes unavailable or conflicting evidence non-countable without a producer, audit, or publication-clock fallback. The released shapes are historical/partial and cannot satisfy a current-profile query.
