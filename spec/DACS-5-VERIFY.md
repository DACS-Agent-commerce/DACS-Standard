...
- **current one-sided timeout blame only** — require exact SPA-8 `participationEvidence`, re-run SPA-1..SPA-6 including source/challenge relations, admission anchor lifecycle, and exact nonresponse/abort outcome evidence; refuse missing, substituted, invalid, or unexpectedly present conditional context.
...
A fifth guard applies to both `derive_authenticated_window` and the composed current-use authenticated-window path: if a one-copy job attributes an ST-9 timeout abort to an absent non-signer, it can enter current metrics only after SPA-1..SPA-6 verify. Authoritative absence, a signed admission, or deadline passage alone cannot satisfy the exact-outcome limb. The standalone AWT path makes a rejected or indeterminate participation/outcome dependency non-countable without manufacturing another fault; the combined CUAW path fails the whole requested operation under its all-job rule. The five pre-current algorithms execute none of this guard and retain their frozen historical behavior.
