...
**Stage:** Verify (5th of 5). **Status:** Draft — **DACS-5 v0.4** on the common DACS v0.1 baseline. v0.3 added `PayeeBoundAgreementDocument` consumption alongside the legacy agreement artifact, the signed `BundleBinding` artifact with BB-1..BB-8 logical→native bundle resolution §10.4.2, and the `FaultAttestationBundle` artifact. v0.4 adds structurally distinct settlement-verified reputation derivation types; the released v0.3 `ReputationDerivation` and `ReplayableReputationDerivation` version-1 semantics remain unchanged. **Depends on:** SR-1 for cross-substrate primary-claim keying, SR-2 for bundle anchoring; composes with the ERC-8004 reputation registry as an OPTIONAL publication surface. **Used by:** all subsequent DACS-1 reputation lookups, external auditors and regulators.
...
- (ii) **divergence → exclusion** — apply the §10.4.3 canonical-divergence predicate, whose `FaultAttestationBundle`-pair rule reads the `outcome` contradiction on the absolute `faultedParty` and outcome class, whose mixed-version rule compares the implied absolute fault, and whose legacy rule compares perspective-reconciled outcomes through `perspective_flip`. For a settlement-verified derivation only, extend that predicate with exact canonical `settlementEvidence[]` reference-multiset equality: an added, removed, duplicated, or substituted full `AttestationRef` excludes the job, while array order alone is immaterial. A job divergent under the applicable predicate MUST be excluded from ALL metrics rather than selecting either copy. Exclusion removes the jobId from both the numerator and `party_fault_denom`; it is not a new bundle outcome;
