...
- (ii) **divergence → exclusion** — apply the §10.4.3 canonical-divergence predicate: copies contradict in `outcome`, in a shared-index `phaseSummary` entry's `kind`/`outcome`/`errorClass`, or by entry presence, subject to the fault/mixed/legacy perspective rules defined there. For a settlement-verified derivation only, extend that predicate with exact canonical `settlementEvidence[]` reference-multiset equality: an added, removed, duplicated, or substituted full `AttestationRef` excludes the job, while array order alone is immaterial. A job divergent under the applicable predicate MUST be excluded from ALL metrics rather than selecting either copy. Exclusion removes the jobId from both the numerator and `party_fault_denom`; it is not a new bundle outcome;
