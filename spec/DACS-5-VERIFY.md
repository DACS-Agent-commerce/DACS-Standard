...
**Authenticated-window current profile (AWT-1..AWT-8; normative).** `AuthenticatedWindowReputationDerivation` is the only current reputation-bearing DACS-5 profile. It is settlement-verified, job-bound, and replayable from inception. The five older derivation shapes remain frozen for compatibility and are historical/partial signals. None can satisfy a request for current-profile reputation.
...
`derive_authenticated_window` is the current-profile path. It applies the same validation, two-sided reconciliation, authoritative-copy selection, RSV admission, outcome formulas, and canonical ordering as `derive_settlement_verified`. It changes the order and source of window admission as follows:
...
| Reputation deriver | For current decisions emit only `AuthenticatedWindowReputationDerivation`: apply RSV-1..RSV-4, require job-bound replay context, and enforce AWT-1..AWT-8 through the shared exact-outcome gate; preserve finalized anchor receipts as provenance rather than occurrence time; preserve every older discriminator as historical/partial without reinterpretation; partition by primary claim; treat failed-substrate per the denominator rule; return null for zero-denominator scalar metrics; set `bundleRefs` to exactly the applicable algorithm's `reconciled` set in canonical ascending-`contentHash` order, and emit a derivation reproducible byte-for-byte from `bundleRefs` plus `resolutionContext` per §10.5.3 |
