...
Before any current-profile BB-5 check, logical-address derivation, resolution-context validation, or reputation replay, the consumer MUST pass CORE §11.1.2 admission for each exact session and authenticated participant whose role evidence it consumes. The release pin, complete module tuple, session, and participant identity come from verifier- or orchestrator-owned trusted context separate from the receipt and dereferenced artifacts. A receipt-carried profile assertion or copied local reference label cannot establish that authority. Missing, partial, malformed, unauthenticated, duplicate, or mismatched authority refuses the current operation before address derivation. An explicitly selected legacy replay may instead preserve the released receipt bytes and frozen historical address semantics, but it makes no current-profile claim and cannot authorize a current lookup, signature, or effect.
