...
- **Write-input mapping.** Where a substrate folds write-time inputs (deployer address, storage-program name, transaction nonce, salt) into its native address — as Demos's StorageProgram derivation does (§6.3.4) — the native address is **not** recomputable from the logical address alone. The implementation MUST then publish the artifact's logical→native binding. For listings, publication is per DACS-1 §6.3.4(b)/(c): descriptive metadata on the anchored record AND the discovery surfaces (§6.3.5 well-known index, §6.3.6 catalog). Listing revocations use `RevocationBinding` and RB-1..RB-6. For DACS-5 bundles, publication is per the §10.4.2 `BundleBinding` rules (BB-1..BB-2). For the DACS-5 LAB activation checkpoint, the active steward publishes a signed `LegacyBundleCheckpointBinding` on the steward's well-known index or a DACS catalog; consumers verify that binding, fetch its native address, and match the resolved checkpoint plus finalized Demos AnchorReceipt exactly as §10.4.1 requires. Consumers resolve the native address through the applicable published binding before reading.
