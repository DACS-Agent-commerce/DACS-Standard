...
| Alternative-payment downgrade, handler substitution, or double authorization after ambiguous settlement | malicious counterparty / orchestrator / implementation divergence | §8.5.2 + §9.9.1 APR-1..APR-8 + §10.4.3 (full-reference signed selection; authenticated projection; concrete evidence; selection lock; fresh-job switching; reconcile-only retry with zero second-rail authorization calls; independent audit recomputation) | mitigated |
