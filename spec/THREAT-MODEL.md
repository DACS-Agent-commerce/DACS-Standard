...
| Sealed-auction selective discovery / omitted better reveal | malicious orchestrator, seller, indexer, or stale node | §8.4.4 SAC-1..SAC-12 (new-type gate; signed bidder records; authenticated current-finalized complete set; exact record accounting; reproducible receipt bound into co-signed agreement) | mitigated for the complete profile; historical §8.4.3 profile does not claim completeness |
| Sealed-auction commitment copy (a copied `{bid, salt}` opening presented under another bidder/channel/job/listing/phase) | malicious bidder | §8.4.4 SAC-11/SAC-12 (context-bound `dacs-sealed-bid-context:v1:` commitment binding jobId, listingRef, phaseIndex, bidderClaim, channelId, bid, and raw salt; historical `dacs-sealed-bid:v1:` frozen and refused for complete-profile records) | mitigated for the complete profile; historical §8.4.3 profile remains context-unbound and discloses that limitation |
| Sealed-auction rule/runtime divergence | malicious rule publisher or implementation mismatch | §8.4.4 SAC-6 (current complete profile admits only byte-specified lowest/highest-price comparison and tie-break; first-acceptable/rule-ref refused before execution) | mitigated for the complete profile |
| Agreement-listing mismatch | malicious counterparty | §8.5.2 (validation in the agreement commitment phase) | mitigated |
| Stronger identity-bound agreement relabelled as, or downgraded to, a historical agreement artifact | malicious counterparty / incompatible implementation | CORE §B.2 IBH-1..IBH-6 + DACS-3 §8.5/§8.6 (distinct discriminators, domains, signed phases, authenticated bundle/CVR joins, and exact five-way dispatch at commit/payment/terminal admission) | mitigated; historical agreement bytes and meanings remain valid under both commitment-record forms and are never normalised or re-signed |
...
| Fresh or backdated legacy agreement, substituted/repriced transition payment, poisoned reservation session, or forged completion audit used after payee-binding activation | malicious orchestrator / counterparty | §8.6 CA-10 + §9.5.1 LAA-1..LAA-7 (governed checkpoint on authenticated immutable session substrate; checkpoint, commitment, role-bound co-signed `LegacyPaymentReservation`, and exclusive signed `LegacyTransitionSettlementEvidence` share one finality/genesis ordering domain; only an exact strictly pre-checkpoint commitment plus strictly pre-checkpoint reservation may complete the actual destination, authenticated payer/payee bundles and authorized paying key, and economics it authenticates before deadline and once per derived idempotency key; full reservation reference/address/receipt verification; transition signer and SR-2 writer bound to retained orchestrator; audit requires the exact key consumed; ordinary settlement evidence cannot be coerced into transition evidence; producer timestamps ignored) | mitigated for current readers; historical and transition bytes remain audit-valid but current-profile-ineligible; deterministic fixtures do not establish native-provider or crash-durable adapter conformance |
| Complete-auction receipt ignored by an older negotiator/payer | minor-version skew | §8.4.4 complete phase kinds + distinct SealedSelectionAgreementDocument/commitment phase + DACS-4 artifact gate + CORE §11.1.2 new-type refusal | mitigated |
...
| Authentic settlement event on a stale fork or behind attacker-selected/fake confirmation head | malicious counterparty / infrastructure | §9.7.0 FV-1..FV-10 (signed rail profile; network/genesis binding; raw inclusion; authenticated head/every ancestry link; computed depth/commitment/BFT quorum; verifier-local freshness) | contract mitigates for finality-bound evidence; repository implementation is fixture-only and production-native codecs remain `indeterminate`; historical evidence is unchanged |
| Invalid or unavailable finality hidden by an outer bundle or weaker copy | malicious producer / compatibility skew | §10.4.1–§10.4.3 distinct finality-bound bundle/pointer domain; role/job authentication before precedence; required strong proof cannot fall back | mitigated by the #392 typed consumer and CUR-3/CUR-4 combined current-use path |
| Post-checkpoint legacy production, forged historical role/mapping, cross-substrate era evidence, or candidate-authorized checkpoint | malicious producer / infrastructure | §10.4 LAB-1..LAB-7 (externally trusted checkpoint discovery; purpose-specific receipt joins; requested-substrate equality; exact original BundleBinding/party-map/budget or pure mapping; strict native ordering before admission) | mitigated for the explicit current-use derivation; unavailable, pruned, reorganized, conflicting, or unorderable authority remains indeterminate |
| Strong reputation request downgraded by discriminator stripping, relabelling, partial-job omission, inconsistent buyer/seller rosters, incomplete historical execution evidence, or claimed-metric replay | malicious producer / compatibility skew | §10.5.1 CUR-1..CUR-8 (exclusive type gate before dereference; closed all-job request; verifier-owned role-map equality; authenticated complete historical nonpayment evidence; no weaker fallback or partial result; complete dependency replay and full-result equality) | mitigated for the unallocated current-use candidate; existing historical derivations retain their frozen weaker contracts |
...
| x402 settlement-reference forgery or required-binding downgrade | malicious server / unavailable infrastructure | §9.5.8 SB-3 (`jobId` + `phaseIndex` bound into the byte-exact `dacs-sb3:v1:` EIP-3009 nonce, or `jobId` into the Permit2 witness; DACS confirms the on-chain settlement, never trusts `PAYMENT-RESPONSE` alone; absent/unavailable required binding is non-countable `indeterminate`, never unbound acceptance or party fault) | mitigated — DACS-side binding, not inherited |
| Cross-job settlement reuse or producer-timestamp winner selection | malicious counterparty / evidence producer | §9.5.8 SB-2 (canonical collision groups; finalized settlement-side binding is authority; without it every competitor is non-countable `indeterminate`; `observedAt`, hash, arrival, and SR-2 publication order are inert) | mitigated within the consumer's presented evidence set; global discovery completeness remains out of scope |
...
| Partial current-use result or favorable outside-window replay context hides a requested job | malicious producer / compatibility skew | §10.5.1 CUAW-1..CUAW-6 (exclusive combined discriminator; LAB/CUR/FV/RSV/SB/LAA/SAC before AWT for every requested job; any indeterminate occurrence fails the whole request; outside-window proof retained in replay) | mitigated only where the verifier supports every applicable dependency and outcome policy; unsupported sealed SAC-8 remains non-passing in the fixture reference |
