...
| Caller-supplied `currentProfile` boolean or copied profile object marks blame/ratings current without verifier authority | malicious counterparty / malicious infrastructure | CORE §11.1.2 + §10.3.2 (verifier-owned corrective-profile admission binding the exact session and authenticated participant identities to the release pin and complete module tuple; caller profile fields inert; absent/unauthenticated/duplicated/mismatched authority fails closed) | mitigated — caller-controlled current-profile labels have no authority |
| Bundle forgery / invented participant in one-sided timeout blame | malicious counterparty | §10.3.2 SPA-1..SPA-6 + §10.11 (co-signature, exact target-signed source-backed obligation, and independently trusted exact nonresponse/abort evidence) | mitigated for current-profile reputation where an applicable outcome authority exists; otherwise audit-only/non-countable |
| Bundle suppression | malicious counterparty | §10.11 + §10.4.3 authoritative absence + §10.3.2 SPA-1..SPA-6 | mitigated for current-profile blame only when absence, exact participation, and exact outcome proofs all verify; otherwise non-countable |
...
| Outsider challenge, signed obligation, receipt-field, or lifecycle substitution | malicious counterparty / malicious infrastructure | §10.3.2 SPA-1..SPA-5 (counterparty/in-roster-orchestrator issuer authorization; closed typed source; unsigned-JCS hash; obligation-derived address; portable receipt with adapter-authenticated native metadata; complete unique incoming-lineage reconciliation) | mitigated subject to configured challenge/source and SR-2 adapter evidence security |
| Invented-roster, abort-session, or stale-outcome rating | malicious counterparty | §10.3.2 SPA-1/SPA-7 + §10.5 AWT gate (global-unique roster; fully signed completed exact rate phase; exact reference; verified outcome occurrence) | mitigated for current-profile reputation; older derivations remain historical/partial |
