...
| x402 settlement-reference forgery | malicious server | §9.5.7 XN-8/XN-9 + §9.5.8 SB-1..SB-3 (DACS confirms the adapter-native settlement event/finality and cross-rail uniqueness; EIP-3009/Permit2 additionally bind the session in authorization; DACS never trusts `PAYMENT-RESPONSE` alone) | mitigated subject to selected scheme/network adapter security |
| x402 receipt reserialization / substitution | implementation bug or malicious server | §9.5.7 XN-7 for negotiated evidence and X402-1..X402-4 for legacy replay (versioned header selection; complete decoded-object NFC+JCS hash; transaction/network cross-check) | mitigated |
| Global x402 rail pins one seller/provider endpoint or leaks operator configuration into protocol identity | malicious infrastructure / implementation divergence | §9.4.3 / §9.5.7 XN-1..XN-3 (discriminated provider-neutral definition; complete request/requirement in the co-signed agreement; facilitator credentials, wallet, RPC and routing remain trusted local configuration) | mitigated for new `x402:protocol` sessions; legacy default disabled/replay-only |
| x402 challenge changes payable request, requirement, amount, destination, or action-bearing extensions after agreement | malicious server / network active attacker | §9.5.7 XN-4..XN-7 (exact single accepts match; signed method/URL/body, payer/payee, scheme/network/asset/timeout/extra/extensions; no redirects; whole-object challenge/receipt hashes before authorization) | mitigated |
| x402 server acknowledgement presented as independent finalized settlement | malicious server / orchestrator | §9.5.7 XN-8..XN-9 (authenticated adapter-native event and finality required for success; unsupported profiles indeterminate; acknowledgement remains diagnostic and cannot authorize delivery/bundles/reputation) | mitigated subject to selected scheme/network adapter security |
