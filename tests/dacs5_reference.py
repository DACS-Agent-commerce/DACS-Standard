...
def _verify_original_pure_mapping(
    ...
    post_ok, _ = _post_fetch_address_valid(
...
def _validate_current_use_absence(job_id, role, substrate, dependencies, verifier_config, role_map):
    ...
    pure_native = _pure_native_address(substrate, logical_address(job_id, role), verifier_config)
    if pure_native is not None:
        ...
        checked = verify_binding(
            binding, verifier_config.get("publicKeys"),
...
def _resolve_current_use_role(job, role, role_request, dependencies, verifier_config):
    ...
    if mapping_kind == "pure":
        ...
        post_ok, post_reason = _post_fetch_address_valid(
            bundle, expected_native, role, bundle_hash(bundle), public_keys,
    ...
    admitted_by_address = {}
    for candidate in survivors:
        checked = verify_binding(candidate, public_keys, expected_jobid=job_id, expected_role=role)
        if not checked["ok"]:
...
def _resolve_current_use_job(job, dependencies, verifier_config):
    ...
    for role in ("buyer", "seller"):
        result = _resolve_current_use_role(job, role, roles[role], dependencies, verifier_config)
...
def derive_current_use_replayable(
    ...
    basis="finalisedAt", computed_at=None,
):
    ...
    try:
        ...
        job_ids = [job.get("jobId") if isinstance(job, dict) else None for job in requests]
        if len(job_ids) != len(set(job_ids)):
            return _current_use_result("error", "requested jobIds must be unique")
        config = dict(verifier_config)
        config["scoredParty"] = party
        admitted = []
        for job in requests:
            result = _resolve_current_use_job(job, dependencies, config)
            if result.get("decision") != "pass":
...
def replay_current_use_derivation(derivation, dependencies, verifier_config):
    ...
    result = derive_current_use_replayable(
        ...
        verifier_config, basis=derivation["windowingBasis"], computed_at=derivation["computedAt"])
