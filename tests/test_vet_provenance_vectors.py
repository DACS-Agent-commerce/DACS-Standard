...
def _validate_vector(case):
    ...
    for item in case["authorizations"]:
        ...
        all_final &= verify_envelope(item, auth_logical(artifact), full_signed=True, gate=gate)
        # Explicit new-type hash separation.
        require(full_hash(artifact) != signing_hash(artifact, "signature"), "authorization hashes must differ")
    ...
    for item in case["composites"]:
        ...
        all_final &= verify_envelope(item, composite_logical(artifact), full_signed=True, gate=gate)
        require(full_hash(artifact) != signing_hash(artifact, "signature"), "composite hashes must differ")
...
class VetProvenanceVectorTests(unittest.TestCase):
    ...
    def test_new_artifact_hashes_are_explicitly_separate(self):
        intentional = {
            "authorization-preimage-hash-is-not-anchor-content-hash",
            "composite-preimage-hash-is-not-anchor-content-hash",
        }
            ...
            for collection in ("authorizations", "composites"):
                for item in vector["input"][collection]:
                    ...
                    if collection == "composites" and "provenancedRecordVersion" not in artifact:
                        continue
                    self.assertNotEqual(signing_hash(artifact, "signature"), full_hash(artifact))
                    if vector["name"] not in intentional:
                        self.assertEqual(item["ref"]["contentHash"], full_hash(artifact))
                        self.assertEqual(item["receipt"]["contentHash"], full_hash(artifact))
